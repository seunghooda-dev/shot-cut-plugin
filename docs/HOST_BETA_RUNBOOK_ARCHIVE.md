...
# 복원 부록(2026-08-06): 소실 절 git 발굴

> 코드 주석이 참조하는 절 16개(§3·§35·§53-i·§101·§104·§104-b·§105-b·§106·§109·§110-b·§111-c·§113·§114·§120·§121-c·§124-b)가 현행 런북과 이 아카이브 어디에도 정의가 없어 git 이력에서 발굴해 복원했다.
>
> 소실 경위 — ①커밋 `97fe032`(2026-07-29 00:29)가 런북 전체를 §122 한 절로 교체하면서 §94~§121-d 시대가 아카이브 이관 없이 삭제됐다. ②이어진 7-29 시대(§122~§132)는 커밋마다 런북이 최신 절만 남기고 통째로 교체돼 각 절이 한두 리비전에만 존재했다(§124-b는 `4db755d` 단 한 리비전).
>
> 분류 — **복원 12** / **정의 절 없음(본문 언급뿐·타표기 현존) 3**(§3·§35·§111-c) / **발굴 실패(이력에 정의 없음) 1**(§53-i). 복원 본문은 소실 직전 리비전에서 **원문 그대로** 가져왔고 수치·판정을 고치지 않았다.

| 참조 | 분류 | 출처 커밋(도입 → 소실) |
|---|---|---|
| §101 | 복원 | `e437f9d` → `97fe032` |
| §104 | 복원 | `b8a9478` → `97fe032` |
| §104-b | 복원 | `56a0cd1` → `97fe032` |
| §105-b | 복원 | `7aa549d` → `97fe032` |
| §106 | 복원 | `4d254b1` → `97fe032` |
| §109 | 복원 | `8ab2338` → `97fe032` |
| §110-b | 복원 | `950597c` → `97fe032` |
| §113 | 복원 | `da7bb3a` → `97fe032` |
| §114 | 복원 | `935b695` → `97fe032` |
| §120 | 복원 | `efe5b18` → `97fe032` |
| §121-c | 복원(§121 하위 절) | `5910270` → `97fe032` |
| §124-b | 복원(§124 하위 절) | `4db755d` → `56c3010` |
| §3 | 정의 절 없음 — 플랜 문서 "## 3." 현존 | — |
| §35 | 정의 절 없음 — 본 아카이브 36-a 현존 | (주석 도입 `01a756c`) |
| §111-c | 정의 절 없음 — §111-a 본문 백로그 언급뿐 | `498a10b` → `97fe032` |
| §53-i | **발굴 실패** — 이력에 정의 없음 | (구현 커밋 `8606741`·`f60dbce`) |

## 부록 A. 정의 절이 없는 참조 4건

### §3 — 정의 절 없음(§ 표기 기준), 실체는 플랜 문서에 현존

- 코드 참조(index.ts 2519줄)는 대상 문서를 명시한다 — "8뉴스 경로는 기본값이라 기존 호출·동작이 그대로다(morning-wide-split.plan.md §3)".
- `docs/01-plan/features/morning-wide-split.plan.md`의 "## 3. 아키텍처 — 프로그램 프로필" 절이 현존하며 내용도 일치한다("8뉴스 프로필 동결 … 8뉴스 경로의 코드 diff 0이 원칙"). 헤더가 `§` 없는 숫자 표기라 § 문자열 대조에서 빠졌을 뿐 소실이 아니다.
- 런북 이력 전체에도 `§3` 표기의 정의 절은 없다(초기 시대는 "## 3. 필수 Smoke Test" 숫자 표기 — 본 아카이브 3절로 현존).

### §35 — 정의 절 없음(리넘버링 어긋남), 실체는 본 아카이브 36-a에 현존

- 코드 참조(src/premiere.ts 1989줄 "(§35 부자연 점프 해소)")의 도입 커밋은 `01a756c`(2026-07-14, "feat: 샷 경계 컷 스냅 + 불확실 경계 마이크로 팬 — '튀는' 전환 해소")다.
- 그 시점 런북의 마지막 절은 "## 35. 얼굴 중심 정밀 프레이밍"이었고, 해당 작업 자체는 직후 "### 36-a. 샷 경계 '튀는' 전환 해소(`01a756c`)"로 등재됐다(커밋 해시로 대응 확인). 즉 주석이 가리키는 내용의 등재 절은 36-a이고, `§35` 표기의 정의 절은 어느 리비전에도 없다.
- "## 35."와 "### 36-a." 모두 본 아카이브에 현존한다.

### §53-i — 발굴 실패(이력에 정의 없음)

- 전 리비전을 통틀어 docs/ 어디에도 `§53-i` 문자열이 등장한 적이 없다(`git log -S"§53-i" -- docs/` 0건).
- 존재하는 것은 구현 커밋뿐이다 — `8606741` "feat: 병합 기사 내부 앵커 컷 자동 분할 §53-i"(index.ts·src/news-cut.ts·tests 변경), `f60dbce` "fix: 앵커 스냅 스캔 창 -12s 확대 + 참조 예시 4장 §53-i". 절 번호가 커밋 메시지와 코드 주석에만 부여됐고 런북 절은 작성되지 않았다.
- 의미 복원은 코드·커밋 메시지로만 가능하다 — 스냅 후 3분 초과 아이템의 내부 앵커 컷 추가 스캔·병합 기사 분할(index.ts 1987·2015줄 주석 참조).

### §111-c — 정의 절 없음(본문 백로그 언급뿐)

- 유일한 등장은 §111-a 본문의 백로그 한 줄이다(도입 `498a10b`, 소실 `97fe032`). 원문 인용 —
  "**6/28X**: 후보 4개로 붕괴(뱅크 미인식)→경고→격자 168프로브→F1 72. §111-a와 무관한 별개 요인 — 이전 실기 100과 소스·시퀀스 차이 재현 조사 필요(§111-c 백로그)."
- 코드에서는 이 번호가 픽스처 명부(scripts/host-smoke/field-fixtures.json — 회차→소스 파일→시퀀스 고정)와 유실 원인 유형화(index.ts 2479·2844·3238줄)에 붙어 있으나, 대응하는 절이 런북에 작성된 리비전은 없다 — 백로그 항목이 절 등재 없이 코드로 구현되며 번호만 쓰였다.

## 부록 B. 복원 절 12개 (원문 그대로)

> 출처 git `5910270`(= `97fe032~1`, 2026-07-28 상태 — 도입 `e437f9d`, 소실 `97fe032`)
...
> 출처 git `5910270`(도입 `b8a9478`, 소실 `97fe032`)
...
> 출처 git `5910270`(도입 `56a0cd1`, 소실 `97fe032`)
...
> 출처 git `5910270`(도입 `7aa549d`, 소실 `97fe032`)
...
> 출처 git `5910270`(도입 `4d254b1`, 소실 `97fe032`)
...
> 출처 git `5910270`(도입 `8ab2338`, 소실 `97fe032`)
...
> 출처 git `5910270`(도입 `950597c`, 소실 `97fe032`)
...
> 출처 git `5910270`(도입 `da7bb3a`, 소실 `97fe032`)
...
> 출처 git `5910270`(도입 `935b695`, 소실 `97fe032`)
...
> 출처 git `5910270`(도입 `efe5b18`, 소실 `97fe032`)
...
> 출처 git `5910270`(이 리비전에서 도입, 소실 `97fe032`) — "## §121 F1 100은 10회차까지 — 11회차에서 짧은 앵커 블록에 무너졌다 (2026-07-28)"의 하위 절
...
> 출처 git `4db755d`(도입·유일 리비전, 소실 `56c3010`) — "## §124 세 번째 붕괴 — 안전망이 거꾸로 작동했다 (2026-07-29)"(2/10 F1 76.9의 원인이 §92 안전망 문턱 3이었음을 규명하고 3→1로 수정한 절)의 하위 절
