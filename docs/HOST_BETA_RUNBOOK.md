...
## §100 배치14 블라인드 실전 — 재인코딩 luma 이동이 검출을 무너뜨린 교란 (2026-07-27)

미학습 신규 회차(7/23)로 일반화 성능을 재는 블라인드 실전. **첫 실행은 실패했고, 원인은 플러그인이 아니라 소스 준비였다.**

- **관측**: 1002s 회차에서 아이템 4개만 생성(통상 12~14). 로그 `앵커 5개(화면 매칭 후보 13 · 학습 모델 0)` — **학습 모델이 시작점을 0개** 검출. 같은 빌드로 하루 전 회차(7/20·7/21·7/22)는 오프라인 F1 100(모델 11·14·12)이라 포맷 변화 가설은 성립하지 않았다.
- **원인**: 배치14를 480p AV1로 받아 `ffmpeg -vf scale=1920:1080 -c:v libx264`로 1080p H.264 변환했는데, 이 변환이 **전 구간 luma를 계통적으로 밀어 올렸다** — 16×9 격자 평균이 118.4→135.0, 137.8→153.2 등 12지점 평균 **+13.6(최대 +23.9)**, 표준편차도 확대. 상관계수는 0.98로 높게 유지되므로 **상관만 보면 놓친다**(선형 변환에 둔감). 참조 뱅크 거리와 학습 모델은 raw luma(144칸 /255)를 그대로 쓰므로 전 화면 +5.5% 이동이 두 경로를 동시에 무너뜨린다.
- **선행 가드는 정상 작동했다**: 변환 전 854×480 원본을 그대로 넣었을 때는 기본 1080p 시퀀스와 프레임 크기가 어긋나 "화면 가장자리가 전 구간 검게 감지" 가드가 분할을 거부했다 — 왜곡된 결과를 만들지 않고 사용자에게 정확한 조치(소스 클립으로 새 시퀀스)를 안내한다.
- **규칙(코퍼스 소스 준비)**: 학습·측정용 회차는 **yt-dlp가 내려준 네이티브 1080p H.264를 재인코딩 없이** 쓴다. 해상도·코덱을 맞추려고 ffmpeg를 거치면 luma 범위가 이동해 측정이 조용히 오염된다. 부득이 재인코딩할 때는 상관계수가 아니라 **격자 평균·표준편차**로 대조할 것(scratchpad/compare-sources.py).
- 480p 프록시는 **오프라인 프레임 판독(FP/FN 라벨 검증)에는 그대로 써도 된다** — 사람 눈 판독은 밝기 이동에 영향받지 않는다. 금지되는 것은 검출 경로에 먹이는 것뿐이다.
