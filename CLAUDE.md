...
| 유형 | 대표 | 상태 |
|---|---|---|
| §125 4형 (앵커 VO + b-roll 직행) | 4/07 220.8 | **수단 소진 재확정(§176-c).** 여덟 경로(후보·모델·오디오·어휘·의미·띠 기하·긴 공백 스윕·화자)에 이어 아홉째(§176 하단 띠 텍스트 주제 전환)도 **실기 기각·원복** — 신호는 유효(재현율 14/14)하나 비전 모델의 주제 판정이 대조에서 2/2 오판(4/07 92.9·1/06 95.7로 악화), §155-c와 동일 실패 모드. 잔여 후보는 §176-d(텍스트 읽기만 시키고 비교는 로컬, 저순위 — 이벤트 열 불완전 선결 필요). 다시 파려면 §176-c부터 읽을 것 |
