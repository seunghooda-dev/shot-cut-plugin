...
발화 밀도/무음 기반 훅 정교화, 실제 "바이럴 점수" 모델. v1은 하이라이트·아웃라인·타임코드만으로 판단.

**완료된 후속(2026-07-14)**: 긴 하이라이트 런의 **문장 경계 분할**(근접도 클러스터 → `splitWindows`로 maxDuration 이내 윈도우, 문장 끝 우선) + 단일 초장 cue의 **워드 단위 스냅**(`wordSnapEnd`, `words[].e` 경계). 유닛 2개 추가(게이트 1618/1618). 다음: 분할 시 **아웃라인(주제) 경계 우선**.
