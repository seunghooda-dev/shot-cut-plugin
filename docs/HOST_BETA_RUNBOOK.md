...
미해결(백로그): 일일 사용량의 날짜 키가 UTC(`toISOString`)라 한국 사용자는 09:00 리셋 — 로컬 날짜 기준으로 바꿀 것(업그레이드 첫 로드에 1회 리셋되는 부작용은 무해). 러너(cdt-blind-run) 감시 필터에 "회수|재심" 누락돼 §110 로그가 안 잡히던 것도 수정됨(스크래치패드).
