...
//
// **스캔 캐시의 출처를 오해하지 말 것(2026-08-10 실측).** `scan-cache/*.json`은 Premiere
// `exportSequenceFrame`이 아니라 **ffmpeg 디코드**에서 나온 격자다. 즉 이 게이트는
// *Premiere에서 뜬* 참조 뱅크와 *ffmpeg에서 뜬* 스캔을 짝지운다(같은 시각 격자가 평균
// 12/255·최대 189 다르다는 실측이 있다). 회귀 감시로는 유효하다 — 캐시가 고정이라 변경 전후
// 비교가 성립하기 때문이다. **절대값을 제품 정확도로 읽지 말 것.**
//
// **캐시가 두 가지 추출 규약을 섞고 있다.** 0722~0804 10회차는 `-ss 0.5 + fps=1`(시각 2…N,
// 실수), 0714~0721·0805~0807 9회차는 `fps=0.5`(t=0부터, 정수)로 만들어져 **위상이 0.5초
// 어긋나 있다**. 스냅샷도 이 캐시로 측정됐으므로 게이트는 자기 일관적이지만, **캐시를 다시
// 만들 때 규약을 섞으면 격자가 최대 247/255까지 달라져 게이트가 무의미해진다**(실측).
...
// 2026-08-11 재측정 — 0715@316·0729@821 결손 라벨 반영(FP→TP). 넓은 창 판독으로 둘 다
// 앵커 명확(4형 아님). 0729 96.8→100.0, 0715 81.3→84.8로 상승 = "검출앵커·라벨 잡음"이
// 정밀도 상한을 눌렀다는 FP 분류 예측의 실증(§7-bl). 이 이득을 회귀 바닥으로 잠근다.
const SNAPSHOT = {
  "20260722_Wed": 100.0, "20260723_Thu": 94.1, "20260724_Fri": 97.4, "20260727_Mon": 100.0,
  "20260728_Tue": 97.9, "20260729_Wed": 100.0, "20260730_Thu": 97.4, "20260731_Fri": 97.3,
  "20260803_Mon": 100.0, "20260804_Tue": 100.0, "20260805_Wed": 91.9, "20260806_Thu": 77.4,
  "20260807_Fri": 100.0,
  // 홀드아웃 6회차(학습·선택 무접촉) — 일반화 감시. §7-ao 시정본 실측.
  "20260714_Tue": 97.0, "20260715_Wed": 84.8, "20260716_Thu": 88.4,
