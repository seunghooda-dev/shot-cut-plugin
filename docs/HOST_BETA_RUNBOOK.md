...
- **남은 가설(미검증)**: ①7/23 회차의 스튜디오·구성이 참조 뱅크와 실제로 다르다 ②Premiere 임포트 경로(프레임 크기·색범위 해석)가 스캔 격자를 코퍼스와 다르게 만든다 ③코퍼스가 7월 회차에 과적합해 하루만 벗어나도 무너진다. **다음 단계는 이 회차의 스캔 캐시를 뽑아 코퍼스 회차와 격자 분포를 직접 대조하는 것**이며, 그 전에는 어떤 결론도 내리지 않는다.
- **의의**: 오프라인 코퍼스 F1 97.9와 별개로, **완전 미학습 회차에서는 무료 경로가 붕괴할 수 있다**는 첫 증거다. 비전 검증은 후보를 제거만 하므로 이 유형의 결손(FN)을 복구하지 못한다 — 상용화 전에 반드시 규명해야 할 항목.
