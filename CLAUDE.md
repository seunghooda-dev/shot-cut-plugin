...
| ③ **실기 비전 ON 블라인드** | **이것이 목표 대상** | **41회차 중 40회차 100**(2026-08-01) · 남은 하나는 4/07 96.3(§125 4형) — 실행 간 동일값(구조 상수, 요동 아님) |
...
| 유형 | 대표 | 상태 |
|---|---|---|
| §125 4형 (앵커 단신 사이 전환) | 4/07 220.8 | **수단 없음, 데이터 보류.** 병목이 §170-b에서 정밀화됐다 — "후보가 안 나온다"가 아니라 ①2초 격자가 어긋나 실제 앵커 프레임(222.0, d=0.160)이 후보로 안 오르고 ②생성된 후보 230.0은 모델 확률 0.106으로 하이브리드 탈락하며 ③통과해도 라벨과 9.2초라 ±8초 밖이다. 문턱 완화는 §161~§163에서 전수 기각 |
| ~~데스크 칼럼 (서서 진행하는 논평)~~ | 7/29 294.3 | **해결(§170·§170-c)** — 비전에 세 번째 답(`standingPresenterOnly`)을 주어 배제된 후보 중 첫 등장만 회수. 82.8 → 100. 회수 경로가 2/19에서 연단 발언자를 칼럼으로 오인해 FP를 냈고(§170-c), 띠 줄 수·배경 단서로 시정했다. **회수 경로 검증에는 과거 FP 이력 회차를 반드시 대조에 넣을 것** — 깨끗한 회차만으로는 위험이 안 드러난다 |
