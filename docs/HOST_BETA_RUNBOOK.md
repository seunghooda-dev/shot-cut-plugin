...
- **처방 방향(미실행)**: ①단기 — 7/23류 앵커 프레임을 참조 뱅크에 추가(가장 싸고 즉효, 다만 새 회차마다 반복되는 대증요법) ②구조 — 모델에서 뱅크 거리 특징을 떼어 두 경로가 독립적으로 실패하게 만들고, 뱅크 실패 시 모델 단독으로도 검출되는지 재학습으로 확인 ③운영 — 뱅크 최소거리가 임계(예: 0.10)를 넘으면 "이 회차는 학습 범위 밖일 수 있다"고 사용자에게 경고. 어느 쪽이든 **홀드아웃을 날짜 무작위가 아니라 "코퍼스에 없는 최신 회차"로 잡아야** 이 실패가 게이트에 걸린다.
