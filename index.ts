...
async function runNewsCutAutoFlow(exportAfter: boolean): Promise<void> {
  ...
  const items = await busy.during("원클릭 분할 · 화면 스캔 준비 중…", async () => {
    ...
    //
    // §107: 발동 조건을 경고 회차(bankFit>0.1)에서 비전 ON 전체로 확장 — 배치15 실측에서
    // 뱅크가 정상인 회차(6/23·6/27)의 FN 4건 중 3건이 100s+ 구간 안(회수 사정권)인데 경고
    // 조건에 막혀 실기에서도 그대로 남았다. 정상 회차의 추가 비용은 리포트 길이 구간 훑기
    // (회차당 프레임 수십 장)이고, FP 위험은 §104 기전 차단·0.9 임계·하단 띠 협동으로 관리된다.
    if (visionEnabled) {
      try {
