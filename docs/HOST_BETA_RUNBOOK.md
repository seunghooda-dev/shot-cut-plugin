...
## §94 FP 라벨 정정 A — 로컬 소스 14회차·19FP 전수 판독, 정정 4건·전체 F1 94.0 (2026-07-24)

fork 세션 권고(경계 보정 보류, 대안: FP 라벨 정정)에 따른 오프라인 사이클. 코퍼스 FP 77건(36회차) 중 **로컬 소스 보유 14회차·19건**을 fps=2 스트립(±6s)으로 전수 판독했다. Host·Premiere 불필요.

- **결과**: 진짜 앵커(라벨 누락) **4건** — 6/02 617.0·679.25(§90 정정본에서도 놓침), 6/15 776.75, 6/16 642.75. 모두 fps=4(0.25s) 스트립으로 컷 확정 후 items.corrected.json 반영(6/15·6/16은 신규 정정본). 해당 3회차 F1 전부 100, 전체 81회차 **93.8→94.0**(TP 867·FP 73·FN 38).
- **나머지 15건은 정정 불가한 진짜 모델 FP로 확정**: 기자 스탠드업(왼쪽 포함) 3·인터뷰/전화CG 3·기자회견/행사 연설 3·유세 군중 2·순수 footage/CG 4. §90 때와 달리 정정 수율이 21%에 그침 — **정정으로 걷을 라벨 누락은 이미 §87~§90에서 대부분 소진**됐고, 남은 FP는 회견·스탠드업류 경성 오탐이라는 뜻. 소스 없는 22회차·58건도 비슷한 수율이면 기대 F1은 ~94.5–95로, 기존 "95~96 전망"은 하향 조정이 맞다.
- **게이트**: 학습은 모델 유지(77.3<84.9), check 1829·check:news 그린.
- 잔여: 소스 없는 22회차·58FP는 재다운로드 배치(수 GB) 필요 — 수율 하향을 감안해 착수 여부는 사용자 판단 대기.

## §95 FP 라벨 정정 B — 소스 유실 22회차 복구·58FP 전수 판독, 전체 F1 94.0→96.7 (2026-07-24)

Premiere 사용 불가 상태에서 가능한 작업으로 배치 B를 완주. **원본 영상 기록이 없어 막혀 있던 22회차를 스캔 캐시만으로 되찾는 경로**를 새로 만들고, 58FP를 전수 판독했다.

- **소스 복구 3단 매칭(신규 도구)**: ①길이 규칙 — 영상 길이 = 스캔 마지막 샘플 시각 **+2~3초**(ID를 아는 로컬 45회차로 실측, 43건 적중). "다시보기" 제목 필터는 진짜 회차를 탈락시켜(6/06·6/07 후보 0) 쓰지 않는다 ②업로드 날짜 — 방송일 당일·익일로 압축(330후보 → 20회차 단일 확정, 2회차만 2후보) ③**luma 격자 대조** — 16×9 gray를 스캔 캐시와 피어슨 상관, 정답 0.945~0.988 vs 오답 0.049~0.243으로 완전히 분리(임계 0.85). 22/22 검증 통과, 애매했던 2회차도 이걸로 갈렸다. 판독용이라 **480p**로만 받아 22회차 1.5GB(타일이 256px라 화질 손실 없음).
- **판독 결과 — 수율 79%(46/58)**: §94의 21%와 정반대다. 원인은 **오래된 회차일수록 라벨이 성기다**는 것 — 1~4월 회차(1/08 4/4, 4/16 3/3, 4/29 3/4)와 7월 초 회차(7/03·7/05·7/06·7/10 각 4건)에서 대량 발견. 남은 12건은 스탠드업·기자회견·CG 설명·오프닝 예고류 경성 오탐(§94와 같은 유형).
- **어긋난 기존 라벨 3건 발견·정정**: 6/24 815→824.25, 7/09 225.75→234.50, 7/19 766.5→755.75. 셋 다 기존 경계가 **b-roll 한가운데**에 찍혀 있어 진짜 앵커 컷과 9~11초 어긋나 있었다(±8s 밖 → FN과 FP를 동시에 유발). 신규 추가 43 + 정정 3 = 46건.
- **효과**: 전체 81회차 **94.0 → 96.7**(TP 913·FP 27·FN 35). 홀드아웃 베이스라인도 84.9→90.7로 올라(라벨 자체가 정확해져) 학습은 모델 유지(84.5<90.7). check 1829·check:news 그린(7/19는 90.9→100).
- **도구 함정 1건**: 컷 0.25s 정밀화에서 탐색 창을 ±1.5s로 잡으면 **b-roll 내부 컷**을 최대 변화점으로 잘못 집는다(5건 실측) — 창을 판독 오차 범위(±0.75s)로 좁혀야 한다.
- **provenance 보존**: `training-data/news-anchor/sources.json`에 회차→YouTube ID 67건을 기록 — 다음 라벨 사이클에서 이 복구 작업을 반복하지 않아도 된다.
- 잔여: FN 35(모델 미검출, 라벨 문제 아님)와 경성 FP 27 — 라벨 정정으로 걷을 수 있는 몫은 이번으로 대체로 소진.

## §96 비전 검증 기본 ON — 키 미설정 사전 건너뛰기 포함 (2026-07-27)

사용자 목표 "F1 100이 되면 비전 상시 ON"의 마무리. §92(오배제 0)·§93(측정 전 회차 F1 100)·회차당 $0.03 미만 실측을 근거로 `news-cut-vision-check` 체크박스를 **기본 checked**로 전환했다.

- **키 미설정 사용자 보호(상용 품질)**: 기본 ON이 되면 키 없는 사용자가 매 실행 프레임 내보내기(68장) 낭비 + 실패 경고를 겪게 되는 문제를 사전 차단 — `hasStoredOpenAIApiKey()`(openai-text.ts 신설, 키 값은 노출하지 않음)로 비전 블록 진입 전에 확인하고, 없으면 info 로그 한 줄("AI 설정 탭에서 저장 시 자동 활성")로 조용히 무료 경로 진행.
- 구현: index.ts 비전 게이트를 `visionEnabled` 변수로 분리(기존 블록 무변경), public/index.html `checked` + 툴팁 갱신. 게이트 check 1829 그린. E2E 러너는 `checked = true` 직접 설정 방식이라 기본 ON과 충돌 없음(토글 아님을 확인).
- 실기 스모크는 Premiere UDT 복구 후 11/26 안정성 실행이 겸한다(체크 상태 기본값만 다르고 판정 경로는 §92 E2E 4회와 동일).

## §97 FN 라벨 전수 재검증 — 삭제 23·이동 2·유지 10, 전체 F1 96.7→97.9 (2026-07-27)

§95의 거울 작업. 잔여 FN 35건을 "모델이 놓친 진짜 앵커"와 "애초에 잘못 찍힌 라벨"로 가르기 위해 전수 판독했다. 1차 판독(±6s 스트립)은 서브에이전트에 위임하고, **라벨 삭제·이동은 전건 넓은 창(-28s~+8s) 재검증을 직접 수행**한 뒤에만 반영했다(정답 데이터 파괴 방지 원칙). 소스 없는 7회차는 §95 복구 파이프라인(길이+날짜+luma 3단, 전부 0.94+ 일치)으로 재확보.

- **결과**: 삭제 23(라벨 시각에 앵커 컷 부재 — 회견·시위·footage 한가운데), 이동 2(6/28 383.75→344.5·845.75→841.0 — 진짜 앵커 컷이 라벨보다 5~39s 앞), 유지 10(진짜 모델 미검출). 전체 81회차 **96.7→97.9**(TP 913·FP 27·FN 12), 재현율 96→**99**.
- **삭제 23건의 두 가지 기원**: ①§95 정정에서 진짜 앵커가 추가되며 드러난 **같은 아이템의 중복 라벨**(구 라벨이 20초쯤 뒤 footage에 찍혀 있던 것 — 1/08·1/12·3/06·6/29·7/07 등 9건) ②앵커 컷 없이 어림잡아 찍힌 경계(경선 개표·시위·의회 표결 등 14건). 이웃 라벨 대조가 필수였다 — 7/01 1005.25는 "이동"으로 보였지만 진짜 앵커(991)는 이미 991.75로 라벨돼 있어 삭제가 정답.
- **유지 10건의 구성 = 모델의 남은 숙제**: 합성 구도 데스크 앵커 7(전부 §92 비전이 아는 유형 — 실기 비전 ON에서는 문제 없음), **스튜디오 비정형 코너 3**(6/30 데스크 칼럼 스탠딩·6/23 대담·6/18은 스탠딩 코너→데스크 복귀 컷은 정상 검출 대상) — 후자는 편집상 실재하는 경계라 라벨 유지, 모델이 못 잡는 known-FN으로 남는다.
- **판독 원칙 추가**: FN 검증은 FP와 달리 **이웃 라벨 목록을 옆에 두고** 해야 한다(앵커 꼬리만 보이는 창은 이동/삭제/정상이 모두 가능 — 이웃이 정답을 가른다).
- 게이트: 학습 모델 유지(84.5<90.7 — 라벨 정화로 홀드아웃 베이스라인 자체가 상승), check:news 그린.
- 잔여: FP 27(회견·스탠드업류 경성 오탐, 실기 비전 ON이 제거) + FN 12(비정형 코너·잔여 미검출). 오프라인 97.9는 **비전 없는 무료 경로** 성적이고, 실기 비전 ON은 §93 기준 측정 전 회차 F1 100.

## §98 11/26 안정성 재실행 — 후보 비결정성 아래에서도 F1 100 재현 (2026-07-27)

§59에서 규명한 "후보 생성 비결정성"(같은 회차·같은 빌드라도 실행마다 후보 집합이 흔들림)이 §92 빌드의 F1 100을 우연으로 만들 수 있는지 확인하는 재실행. 11/26을 §96 빌드로 한 번 더 완주시켰다.

- **결과 F1 100(TP 13·FP 0·FN 0)** — 후보 16개 중 비전이 104.0·718.0·826.0 세 건을 배제해 13개 확정, 정답 13아이템과 1:1. **오배제 0**(배제 3건 모두 정답 경계에 없는 진짜 FP)이고 시퀀스 생성 13 성공·0 실패.
- 배제 3건은 §93 실행과 동일한 집합이었다 — 후보 비결정성이 있어도 **비전 판정이 같은 결론으로 수렴**한다는 첫 반복 증거(1회 → 2회 재현). 비결정성은 후보 단계에 남아 있으나 최종 경계에는 전파되지 않는다.
- 운영 메모: UDT 연결 실패의 원인은 사용자 환경이 아니라 스모크 러너였다 — `connectPanel`이 앱 clientId를 1~6만 훑는데 clientId는 서비스 접속마다 증가하므로 장시간 세션에서는 범위를 넘어간다. 1~60 스캔으로 수정(커밋 2a06c5e). **연결 실패를 사용자 설정 탓으로 먼저 의심하지 말 것.**

## §99 릴리스 리뷰 — 비전 기본 ON의 동의·옵트아웃 결손 시정 (2026-07-27)

§96(기본 ON)을 상용 기준으로 재검토한 결과, **기본값 전환이 만든 방어선 붕괴**를 발견했다. 기능 자체가 아니라 "기본값이 바뀌면 무엇이 함께 무너지는가"를 묻지 않은 것이 원인이다.

- **C-1 AI 전송 동의 우회(출시 차단급)**: 이 저장소의 다른 11개 OpenAI 경로는 전부 `ensureAiConsent`를 먼저 거치는데 **원클릭 비주얼 분할만 예외였다.** 기본 OFF 시절에는 "사용자가 체크박스를 켜는 행위"가 불완전하나마 동의를 대신했지만, 기본 ON이 되면서 동의 화면을 한 번도 본 적 없는 사용자가 버튼 한 번에 자기 방송 프레임 수십 장을 제3자에 전송하게 됐다. 동의 문구(`public/index.html`)가 명시적으로 "**이미지**…가 OpenAI API로 전송될 수 있으며"라고 약속한 바로 그 대상이다.
  - **시정**: 비전 블록 진입 전 `settings.aiConsentAccepted` 확인. 다른 경로와 달리 **예외를 던지지 않는다** — 무료 분할까지 막으면 안 되므로 비전만 건너뛰고 활동 로그 한 줄로 알린 뒤 완주시킨다(키 미설정 가드와 같은 모양).
- **H-2 옵트아웃 미영속**: 체크박스가 어떤 설정에도 저장되지 않아 매번 HTML 기본값으로 리셋됐다. 기본 OFF일 때는 무해했지만(꺼짐이 유지됨) 기본 ON에서는 **비용을 피하려고 끈 선택이 패널 재적재마다 조용히 무효화**된다. `PluginSettings.newsCutVision`(기본 true, `!== false`로 정규화) 신설 + 저장/복원 배선.
- **M-2 비용·전송량 고지**: 금액이 hover 툴팁에만 있고 전송 분량 표기는 §96이 지우면서 복원되지 않았다. 가시 `field-hint` 추가(약 40~100장·수십 원·건너뛰기 조건), 툴팁에 정확한 분량(후보 1개당 4프레임) 복원, 비전 **시작** 시점 `activity.add` 한 줄 추가(진행 표시는 사라지므로 사후 감사 기록이 로그에만 남는다).
- **M-3 계약 테스트**: `tests/ui-contract.test.ts`가 `ai-consent-checkbox`의 기본 미체크는 못 박고 있었는데 새 기본값에는 아무 계약이 없었다. HTML `checked` 속성과 `DEFAULT_SETTINGS.newsCutVision`을 **한 단언으로 묶어** 둘이 어긋나면 게이트가 잡도록 고정.
- **H-3 스모크 러너 소켓 누수**: `connectPanel` 실패 경로가 서비스 웹소켓을 닫지 않았다 — 접속 하나가 clientId 하나를 소비하므로 재시도마다 앱 clientId를 밀어내 스캔 범위를 스스로 넘기는 구조였다(누수가 원인을 재생산). 실패 시 `service.close()` 추가. 스캔 범위 확대로 최악 실패 대기가 3.7분→10분이 된 것은 사실이며, 2a06c5e 커밋 메시지의 "총 대기시간 유지"는 후보당 타임아웃만 본 오산이었다.
- **미해결로 남긴 것 — H-1 AI 큐 우회**: 비전 경로가 `aiQueueController`를 거치지 않아 일일 한도·비용 단위·승인 임계가 적용되지 않고 "오늘 사용" 카운터에도 잡히지 않는다. 이 우회는 §96이 만든 것이 아니라 원래 있던 것이지만, 옵트인일 때는 사용자가 고른 예외였고 기본 ON이 되며 **"비용 보호가 기본으로 무력화된 유료 기능"**이 됐다. 큐 경유는 배치 작업의 승인 단위 설계가 필요해 별도 작업으로 남긴다.
- **교훈**: 기본값 전환은 한 줄짜리 변경으로 보이지만 **그 기능을 감싸던 암묵적 방어선(사용자의 의도적 클릭)을 함께 제거한다.** 옵트인→기본 ON 전환 시에는 동의·비용·옵트아웃 영속·고지 네 가지를 반드시 함께 점검할 것.

## §100 배치14 블라인드 실전 — 신규 회차 검출 붕괴(모델 0), 소스 가설은 대조군으로 기각 (2026-07-27)

미학습 신규 회차(7/23)로 일반화 성능을 재는 블라인드 실전. **결과는 실패이고, 원인은 아직 미규명이다.**

- **관측**: 1002s 회차에서 아이템 4개만 생성(통상 12~14). 로그 `앵커 5개(화면 매칭 후보 13 · 학습 모델 0)` — **학습 모델이 시작점을 0개** 검출, 참조 매칭 후보도 13(11/26은 41). 같은 빌드로 직전 회차 7/20·7/21·7/22는 오프라인 F1 100(모델 11·14·12).
- **기각된 가설 — 재인코딩 luma 이동**: 배치14를 480p AV1로 받아 1080p H.264로 변환해 넣었으므로 "변환이 luma를 밀어 검출을 깨뜨렸다"고 의심했다. 변환본을 **AV1 원본과** 비교하니 격자 평균이 12지점 평균 +13.6(최대 +23.9)이라 확증된 것처럼 보였다. **그러나 이는 대조군을 빼먹은 오판이었다** — 네이티브 1080p H.264를 따로 받아 변환본과 직접 비교하니 **상관 0.9999·평균 밝기 차 0.1**로 사실상 동일했다. +13.6은 Premiere에 넣은 적조차 없는 AV1 480p 스트림이 (같은 마스터의 H.264 스트림보다) 어둡게 디코딩되기 때문에 생긴 값이고, 변환본은 코퍼스가 쓰는 네이티브 H.264와 같은 밝기다. **소스는 무죄다.**
  - **판독 규칙**: 두 파일의 luma를 비교할 때는 **실제로 파이프라인에 들어간 두 파일**을 비교해야 한다. "원본과 비교"는 원본이 파이프라인에 들어가지 않는 한 무의미하다. 그리고 상관계수는 선형 변환에 둔감하므로 **평균·표준편차를 함께** 볼 것(scratchpad/compare-sources.py).
- **선행 가드는 정상 작동했다**: 변환 전 854×480 원본을 그대로 넣었을 때는 기본 1080p 시퀀스와 프레임 크기가 어긋나 "화면 가장자리가 전 구간 검게 감지" 가드가 분할을 거부했다 — 왜곡된 결과를 만들지 않고 사용자에게 정확한 조치(소스 클립으로 새 시퀀스)를 안내한다. 상용 관점에서 이 실패는 바람직한 실패다.
- **원인 확정(실제 스캔 캐시 대조)**: 7/23 시퀀스의 Premiere 스캔 캐시를 뽑아 7/22 캐시와 **같은 매처·같은 모델**에 태웠다. 두 회차 모두 뱅크0으로 라우팅됐으나 매칭 품질이 갈렸다.

  | | 7/22(정상) | 7/23(붕괴) |
  |---|---|---|
  | 뱅크 최소거리 | 0.0784 | **0.1179** |
  | 모델 확률 최대 | 0.983 | 0.858 |
  | τ0.75 초과 표본 | 81 | **2** |
  | 참조 후보 / 모델 시작 | 31 / 12 | 16 / **0** |

  참조 뱅크가 이 회차 앵커샷을 50% 나쁘게 매칭했고, τ 초과 표본이 2개뿐이라 `detectModelStarts`의 "연속 2표본" 조건을 만족하는 구간이 없어 모델 시작이 0이 됐다. 스튜디오 포맷 자체는 정상이다(62s·860s 프레임 육안 확인 — 왼쪽 앵커·합성 배경, 평소와 동일).
- **구조적 결함 — 두 검출 경로가 독립이 아니다**: 참조 매칭과 학습 모델은 서로를 보완하는 이중화로 설계됐지만, 모델의 특징 벡터에 **뱅크 거리가 그대로 들어간다**(`news-visual-cut.ts:240` — `z += w * min(1, matcher.distance(grid) * 4)`). 그래서 뱅크가 빗나가면 후보 생성과 모델이 **같은 원인으로 동시에** 무너진다. 이번 회차가 아이템 4개로 끝난 이유가 이것이다. 이중화가 공통 원인을 공유하면 이중화가 아니다.
- **기여도 정량화 — 모델은 뱅크 거리의 얇은 껍데기다**: 뱅크 거리 가중치 `w[146] = -4.789`(bias 2.560)로 단일 특징 중 압도적이다. 이 항만 0으로 두고 같은 가중치로 재채점하면 **두 회차 모두** τ0.75 초과 표본이 폭증한다.

  | | 원래 τ초과 / 모델시작 | 뱅크항 0 τ초과 / 모델시작 |
  |---|---|---|
  | 7/22 | 81 / 12 | 314 / **50** |
  | 7/23 | 2 / **0** | 303 / **57** |

  즉 나머지 144개 luma 가중치는 **단독으로는 앵커를 전혀 가르지 못한다**. 학습 모델이 참조 매칭을 보완하는 두 번째 경로처럼 보였지만, 실제로는 같은 신호를 시그모이드로 감싼 것에 가깝다.
- **처방 ② 기각(실험 완료)**: "뱅크 거리 없이 처음부터 재학습하면 luma 가중치가 판별력을 얻는가"를 실제로 돌렸다(학습 77회차·홀드아웃 5회차, 특징 146을 0으로 두고 4개 설정 스윕, 추론도 같은 조건).

  | 설정 | 홀드아웃 F1 | 7/23 |
  |---|---|---|
  | **기준선(뱅크 특징 포함)** | **82.6** | 47.1 |
  | 뱅크 제거 ep800 lr1 pw8 l2=0 | 38.9 | 20.7 |
  | 뱅크 제거 ep800 lr1 pw8 l2=1e-4 | 59.5 | 27.6 |
  | 뱅크 제거 ep1500 lr0.6 pw8 l2=1e-4 | 77.8 | **44.4** |
  | 뱅크 제거 ep1500 lr0.6 pw4 l2=1e-4 | 70.6 | 47.1 |

  **어느 설정도 기준선을 넘지 못했고, 목표였던 7/23조차 개선되지 않았다.** 따라서 문제의 성격이 다르다 — "모델이 뱅크에 잘못 결합돼 있다"가 아니라 **현재 특징 공간(144 luma + 전후 프레임차)에 앵커를 가릴 정보가 뱅크 신호 외에는 없다**. 이중화가 아니었던 이유도 이것이다. 결합을 끊는 방향은 막다른 길이고, **뱅크를 더 잘 일반화시키거나(①) 근본적으로 다른 특징을 추가하거나 한계를 인정하고 알리는(③)** 쪽만 남는다.
- **남은 처방**: ①**뱅크 다양성 보강** — 새 회차 앵커를 참조에 추가. 즉효지만 배포판 사용자는 뱅크를 갱신할 수단이 없어 근본 해결이 아니다(제품이 스스로 참조를 늘리는 경로가 필요). ③**거리 임계 경고**(아래 타당성 검증 완료) — 실패해도 손해가 없는 안전망. ④(신규) **특징 추가** — 얼굴·인물 위치, 하단 띠 기하, 오디오 등 luma 밖의 신호. 비용이 크므로 별도 설계가 필요하다. 어느 쪽이든 **홀드아웃을 날짜 무작위가 아니라 "코퍼스에 없는 최신 회차"로 잡아야** 이 실패가 게이트에 걸린다.
- **정답 라벨 확정 → 수치화**: 7/23을 전수 판독해 **12아이템**으로 라벨했다(리포트 6개 105~152s + 단신 6개 30~42s, 아웃트로 982.44s). 1차 판독은 서브에이전트, 놓쳤다는 경계 3건(497.91·758.38·913.75)은 직접 컷 전후 프레임으로 재확인했다(전부 footage→왼쪽 앵커 전환 확인).

  | | 코퍼스 81회차 | **신규 7/23** |
  |---|---|---|
  | 오프라인(무료 경로) | 97.9 | **47.1** (TP 4·FP 1·FN 8) |
  | 비전 ON 실기 | 100(측정 3회차) | **50.0** (TP 4·FP 0·FN 8) |

  **비전은 여기서도 정상 작동했다** — FP 1건(382.0, 아이템 3 내부 footage)을 정확히 제거해 오배제 0을 유지했다. 결손은 전부 FN이고 비전은 후보를 더하지 못한다. 실패 패턴이 뚜렷하다: 놓친 8건 중 **6건이 후반 30~42초 단신 블록**이다.
- **홀드아웃 재설계(적용함)**: `train.mjs`의 `DEFAULT_HOLDOUT`에 `Train_KBC_20260723_Thu`를 추가했다. 기존 홀드아웃 4회차는 전부 **학습 분포 안의 미측정 회차**라 일반화 실패를 구조적으로 못 잡는다. 새 회차를 코퍼스에 넣기만 하고 홀드아웃에 고정하지 않으면 다음 학습이 삼켜 **실패가 지표에서 사라진다** — 이번 건이 정확히 그 함정이었다.
- **처방 ③ 타당성 검증(실측·구현 전)**: "뱅크 최소거리가 회차 단위 실패를 예측하는가"를 전 82회차에서 확인했다. 실패(F1<80)는 2건뿐이고 — 20260723_Thu(거리 0.118·F1 47)·20260628_Sun(거리 0.100·F1 73) — 둘 다 거리 상위권이다.

  | 임계 | 경고 대상 | 그중 실제 실패 | **놓치는 실패** |
  |---|---|---|---|
  | 0.09 | 13회차 | 2 | **0** |
  | **0.10** | **6회차** | **2** | **0** |
  | 0.11 | 1회차 | 1 | 1 |

  **임계 0.10이 실패 2건을 모두 잡으면서 오경보는 4건**이다(82회차 중). 경고는 검출 동작을 바꾸지 않으므로 이 오경보율은 감수할 만하다. 다만 거리가 높다고 항상 실패하는 것은 아니다 — 20260603_Wed는 거리 0.1095에 모델 시작 0인데도 뱅크 후보만으로 F1 100이다. 즉 거리는 **위험 신호이지 실패 판정이 아니다**. 문구도 "실패했다"가 아니라 "이 회차는 학습 범위 밖일 수 있으니 결과를 확인하라"여야 한다.
- **처방 ③ 구현 완료(커밋 a7d13c1)**: `bankFitDistance()` + `BANK_FIT_WARN_DISTANCE=0.1`. 검출 동작은 그대로 두고 실행 초반에 경고 한 줄. 임계값은 계약 테스트로 고정했다(무심코 바뀌면 경고가 전 회차에 뜨거나 사라진다).
- **왜 뱅크가 빗나가는가 — 매처가 앵커가 아니라 화면 전체를 본다**: 사용자 제안("앵커의 대략적 xy 좌표를 쓰면 되지 않나")을 계기로 가중치 분포를 실측했다. 가중치는 `1/(√std+8)`로 **참조들끼리 일정한 셀일수록 커지는데**, 배경 셀이 99/144개라 **총 가중치의 68%를 배경이 차지한다**(칸당 가중치는 앵커 0.0759 vs 배경 0.0733으로 거의 동일). 배경은 매 컷 달라지므로 같은 앵커샷도 배경 운에 따라 통과 여부가 갈린다 — 놓친 8샷과 잡은 4샷이 육안으로 구분되지 않는 이유다.
  - 셀별 차이 히트맵에서 7/23은 **하단 2행**이 특히 어긋난다(7/22 맨 아랫줄 10~26 vs 7/23 27~62). 컷 직후 헤드라인 배너가 아직 안 뜬 구간이 섞이는 탓이다.
  - 왼쪽 6열만 써도 7/23 앵커는 참조에서 0.140, 7/22 앵커는 0.097 — **배경을 걷어내도 7/23 앵커 영역 자체가 44% 더 멀다.** 영역 제한은 필요조건이지 충분조건이 아니다.
- **영역 제한 매처 본실험(영역 6종 × 하이퍼파라미터 5종 = 30회 학습, 각 영역 최적을 82회차 전량 재평가)**

  | 영역 | 홀드아웃 5 | **전량 82** | 7/23 | F1<80 회차 |
  |---|---|---|---|---|
  | **전체화면(현행)** | 75.9 | **94.4** | 47.1 | **4건** |
  | 열6 행0~8 | **80.0** | 91.2 | **75.0** | 11건 |
  | 열8 행0~8 | 75.3 | 91.5 | 60.0 | 9건 |
  | 열7 행0~8 | 71.6 | 91.0 | 57.1 | 10건 |
  | 열5 행0~8 | 73.1 | 90.3 | 57.1 | 13건 |
  | 열6 행0~7 | 74.2 | 89.8 | 50.0 | 13건 |
  | 합집합(전체화면 ∪ 열6) | 72.1 | 91.8 | **37.5** | 7건 |

  **홀드아웃만 보면 영역 제한이 이기지만(80.0>75.9) 전량에서는 진다(91.2<94.4).** 망가진 회차가 4건→11건으로 늘고 2/24는 53, 3/06은 42로 크게 깨진다 — **1회차 살리자고 7회차를 깨는 교환**이라 그대로 채택할 수 없다. 배경 정보는 많은 회차에서 실제로 일을 하고 있다.
  - **합집합은 더 나쁘다(7/23 37.5)** — 두 매처의 거리 척도가 달라 후보 목록을 합치면 `hybridAnchorTimes`의 적응 임계가 망가진다. 섞으려면 후보 단계가 아니라 **파이프라인을 두 번 돌려 아이템 경계 수준에서 병합**해야 한다.
  - 7/23 회복은 후보 생성이 한 것이고 모델 시작은 1개뿐이다 — **모델은 어느 영역에서도 뱅크에 얹혀 있다.**
- **종합 결론 — 고정 뱅크의 용량 한계**: 참조 13장 + 하나의 전역 가중치로는 **회차별 배경 변화와 회차별 앵커 렌더링 차이를 동시에 감당할 수 없다.** 영역을 좁히면 배경 강건성을 잃고, 넓히면 앵커 특이성을 잃는다. 어느 고정 설정도 두 축을 함께 만족시키지 못했다. 따라서 처방은 설정 튜닝이 아니라 **뱅크가 자라는 구조**여야 한다(→ 처방 ① 설계, `docs/01-plan/features/news-anchor-reference-growth.plan.md`).
- **의의**: 오프라인 코퍼스 F1 97.9는 **미학습 회차에 대해 심하게 낙관적**이다(실측 47.1). 상용 배포판이 만나는 것은 코퍼스 회차가 아니라 새 회차이므로, 이 격차를 좁히기 전에는 F1 100을 제품 성능으로 말할 수 없다.

## §101 비전 제안자 실험 — 무료 신호 천장(63.2)을 비전이 5/5로 돌파 (2026-07-27)

§100에서 무료 신호(뱅크 거리·컷 크기)로는 7/23의 놓친 경계를 어떤 파라미터로도 3개까지밖에 못 찾는다는 것이 확정됐다(oracle 천장 63.2). 판별 정보가 **픽셀에는 있고 luma 격자 표현에는 없다**는 가설을 검증하기 위해, 검출이 무너진 구간(400~870s)을 10초 간격 48프레임으로 훑어 **배포 프롬프트 그대로** 판정시켰다.

- **결과: 놓쳤던 경계 5개 전부 포착.** 비전이 앵커라고 답한 프레임은 400·410·500·510·630·640·760·800·840이고, 경계로 환산하면 392.81·497.91·624.54·758.38·799.63·833.79 — **6개 경계 전부 정확, 48프레임 중 헛짚음 0건.**
  - 스크립트가 출력한 "정밀도 67%"는 판정 창(+14s)이 좁아서 생긴 착시다. 400·410은 392.81 앵커 리드이고, 770·810은 30~40초짜리 단신이라 그 시점엔 이미 리포트로 넘어간 것이 맞다. **실질 오검출 0.**
- **의미**: 뱅크가 "무작위 프레임보다 덜 앵커답다"고 본 758.38(거리 0.1991, 회차 중앙값 0.1803보다 나쁨)을 비전은 정확히 앵커로 판정했다. 두 신호가 보는 것이 근본적으로 다르다는 직접 증거다.
- **비용**: 48프레임(기존 68장 대비 +71%). 안전망(§100 처방 ③, `bankFitDistance > 0.1`)이 발동한 회차에만 붙이면 **정상 회차 비용은 0**이다.
- **남은 과제(미검증)**: ①10초 격자로는 경계가 최대 ±5s 어긋난다 — 기존 0.25s 재스냅이 이를 흡수하는지 확인 필요 ②정상 회차에서도 오검출 0인지(이번 측정은 실패 회차 1건) ③단신 구간(30~40s)은 10초 격자가 성길 수 있어 간격 튜닝 필요 ④제안자 모드는 "제외에 보수적"인 현행 튜닝과 성향이 반대라, 임계·투표 규칙을 별도로 잡아야 한다.
- 실험 스크립트: scratchpad `propose-test.mjs`(키는 패널 secureStorage 밖으로 나가지 않는다 — 판정 전량 패널 컨텍스트 실행).

## §101-b 회수 경로 실기 반복 — 배치 상한·짧은 리드·배경 인물 오독 3종 시정 (2026-07-27)

§101의 회수 경로를 7/23 실기에서 3회 반복하며 결함을 하나씩 걷어냈다. 각 실행이 서로 다른 층의 문제를 드러냈다.

- **1차 E2E — 회수 0건**: 훑기 59프레임·판정 57까지 진행됐으나 추가 경계 0. 원인은 배치를 장수로만 나눈 것 — `classifyAnchorShots`의 합계 1.2MB 상한에 **전 배치가 조용히 걸려** 내 catch가 삼켰다. 검증 경로가 바이트+장수로 나누는 데는 이유가 있었다(§78). 바이트 분할 + 실패 배치 수 경고 가시화로 수정(a0fb00b). **침묵하는 실패 경로를 만들지 말 것 — 실패는 반드시 개수로 드러낸다.**
- **2차 E2E — 회수 첫 작동, 그러나 3/5**: 배치 실패 0, 79프레임 훑기 → 경계 3개 추가(504·634·834), 아이템 4→7. 재스냅이 504→497.9, 634→624.5로 정확히 당겼다(±10s 격자 오차 흡수 검증 완료). 그러나 오프라인 5/5였던 회수가 3/5에 그쳤고, **834 오배제가 2연속 재현**됐다(같은 실행에서 회수는 834를 anchor 0.9+로, 검증은 835.2·838·841을 non-anchor로 — 명백한 모순).
- **패널 내보내기 프레임 전수 육안 판독으로 원인 이분**(프레임 내보내기는 무죄).
  - **① 짧은 리드 격자 이탈**: 764·922·952 프레임은 실제로 이미 footage였다 — 단신 앵커 리드가 **5~8초**뿐이라 10초 격자가 통째로 건너뛴다(758.4·913.8·944.5). 판정은 옳았고 격자가 성겼다. → step 5s(≥5s 리드에 프로브 보장)·maxProbes 200.
  - **② 배경 인물 오독**: 835.2·838·841(정청래 연단 배경)·804(쇼벨 두 인물)·362(안도걸 연단)는 **명백한 앵커샷인데** non-anchor로 판정됐다. 연단 발언·기자회견이 배경으로 합성되면 모델이 "기자회견 footage"로 오독한다 — 834 오배제 2연속의 원인. → 프롬프트에 "배경 속 인물은 판정 근거가 아니다, 전경의 착석 진행자만 본다" 절 추가(§92 합성 구도 절과 같은 방식, 검증·회수 공용).
- **비용 실측**: 2차 기준 검증 24 + 회수 79 = 103프레임. 5s 격자로 ~187프레임 — 경고 회차에만 붙는 비용이고 정상 회차는 0.

## §101-c 같은 회차 참조·배제 재심 — 7/23 실기 F1 100 도달 (2026-07-27)

§101-b 이후 남은 결손(배경 인물 합성 오독)을 같은 회차 확정 앵커 참조로 풀었다. 7/23 실기 5회 반복의 F1 궤적: **47.1 → 73.7 → 90.9 → 95.7 → 100**.

- **같은 회차 참조 주입(6783b92)**: 회수 시점에는 비전 검증을 통과한 확정 앵커가 있다 — 앞쪽 3개의 +1.2s 프레임을 내보내 참조 앞자리에 넣는다(코퍼스 예시는 뒤로, 합계 5장). 4차 실기에서 정청래 배경(833.8)이 회수됐고 홀드아웃 전체 11/12(95.7)로 상승. footage가 참조로 들어갈 위험은 없다(verified는 검증 통과분).
- **4차의 이상 — 검증 오배제 확대**: 834(4연속)에 872(진짜 경계)까지 배제. 회수가 둘 다 재추가해 자기치유했지만 그건 배제 지점이 우연히 긴 구간 격자 안에 있었던 덕이다. **배제 후보 시각(+1.2s)을 회수 프로브에 직접 추가**해 구조로 만들었다(b4b952f·f0feafe) — 같은 회차 참조가 포함된 회수 판정이 재심 역할.
- **799.63(쇼벨 배경) 진단**: 같은 회차 참조 + 현행 프롬프트로 3회 반복 판정 시 리드 4프레임 전부 anchor 0.98~0.99 만장일치 — 오독도 임계 미달도 아니었다. 4차 미회수의 원인은 미규명(배치 구성 효과 의심)이나, 5차에서 804가 정상 회수되며 해소. 진단 로그 2종(내보내기 유실 지점·임계 미달 판정)을 상시화해 다음 이상 때 즉답하게 했다.
- **5차 실기(최종)**: 후보 5 → 검증 배제 1(382 footage, **오배제 0**) → 회수 8 추가(804 포함) → **아이템 12 = 정답 12, F1 100·FP 0**. 유실 2장(160·674)은 아이템 내부 footage라 무해함을 진단 로그로 확인.
- **게이트 위반 사고(정직 기록)**: b4b952f는 typecheck 실패 상태로 커밋·푸시됐다 — `check | grep` 파이프라인의 종료 코드가 grep의 것이라 실패가 가려졌다. f0feafe로 시정. **게이트 판정은 파이프 없이 종료 코드로 직접, 커밋은 별도 명령으로.**
- 완료 조건 진행: 미학습 신규 회차 실기 F1 100 — **7/23 달성(1/3)**. 다음: 7/24(라벨 10아이템 확정)·7/25(11아이템 확정, 토요일 포맷).

## §102 미학습 신규 회차 3연속 실기 F1 100 — 목표 조건 충족 (2026-07-27)

CLAUDE.md에 명문화한 완료 조건("미학습 신규 회차 3회 연속 실기 비전 ON F1 100 + 코퍼스 전량 무하락 + 오배제 0")을 배치14 3회차로 검증했다. 세 회차 모두 라벨은 사전 확정(서브에이전트 1차 판독 + 경계 전건 본인 재검증)하고 실행은 블라인드였다.

| 회차 | 경로 | 결과 |
|---|---|---|
| 7/23(목) | **회수 경로** — 경고 발동, 후보 5 → 검증 배제 1(footage) → 회수 8 추가 | **12/12 · F1 100 · FP 0** |
| 7/24(금) | 기본 경로 — 뱅크 정상(모델 10), 경고·회수 미발동 | **10/10 · F1 100 · 배제 0** |
| 7/25(토) | 기본 경로 — 신형 포맷 라우팅 정상(모델 11) | **11/11 · F1 100 · 배제 0** |

- **의미**: 뱅크가 무너진 회차(7/23)와 정상 회차(7/24·7/25)가 서로 다른 경로로 같은 결과에 도달했다 — 안전망·회수는 필요할 때만 발동하고, 정상 회차에 비용·위험을 더하지 않는다.
- **코퍼스 무하락**: 전량 82회차 오프라인 97.4는 7/23(47.1)이 코퍼스에 새로 편입된 효과다. 동일 회차 기준(7/23 제외 81회차)으로는 TP 913·FP 27·FN 12 = **97.9로 §97과 동일** — 오늘 변경(전부 비전 경로)의 무료 경로 회귀 없음.
- **오배제 0**: 3연속 실행의 배제는 7/23의 382(진짜 footage) 하나뿐. 834 오배제는 4차까지 재현됐으나 5차엔 미발생 + 배제 재심 구조(§101-c)로 상시 방어.
- **7/24 소스 주의**: 채널에 1080p H.264 스트림이 없어 720p로 받힘 — 1080p 업스케일 재인코딩 필요했다(H.264 업스케일은 luma 동일, §100 확립). 필러박스 가드가 720p 원본을 정확히 거부하는 것도 재확인.
- **판정 주의(정직)**: 3회차 라벨은 오늘 만든 것이라 라벨러(=나)와 판정자가 같다. 라벨은 실행 전 확정·커밋했고 경계 전건을 프레임으로 재검증했지만, 독립 검증자는 아니다. 또한 "3회 연속"은 이 3회차 기준이며 다음 신규 회차(7/26 이후 업로드분)가 진짜 지속 시험이다.
- 다음: 7/24·7/25 스캔 캐시 코퍼스 편입(오프라인 재현), 신규 회차 나오는 대로 지속 검증, H-1 큐 우회 설계.

## §103 6/28 회수 실기 — 회수 경로가 라벨 결손을 적발, 정정 후 95.7 (2026-07-27)

홀드아웃의 두 번째 실패 회차(구 라벨 기준 F1 72.7)에 회수 경로를 실기로 돌렸다. 결과가 라벨 검증 사이클을 촉발했다.

- **실행**: 경고 발동(일치도 0.100 — 임계에 정확히 걸침), 후보 4(배제 0) + 회수 8 = 아이템 12. 구 라벨(7아이템) 기준 "TP 7·FP 5"였는데, FP 다섯 지점의 프레임이 전부 **새 헤드라인의 앵커샷**이었다.
- **전면 재판독 → 구 라벨이 틀렸다**: 137.5~233.5가 2개, 626.5~841이 4개 아이템을 뭉침 + 491.25는 누리호 footage 한가운데. 정정 = 신규 4(179.5·739.5·768.25·805.75)·이동 1(→494.25), **11아이템**. 신규·이동 5건 전부 컷 전후 프레임 직접 재검증.
- **정정 후 판정**: 실기(회수 포함) **TP 11·FP 1·FN 0 = 95.7**(구 경로 72.7). 무료 경로는 53.3 — 모델이 일요일 단신 블록을 못 잡는 알려진 한계로, 회수가 이를 메운다. 전량 84회차 97.3(정답 아이템 수 증가로 분모 확대).
- **잔여 FP 1건(27.25)의 기전**: 9.5~35.8이 컷 없는 단일 테이크(인사→아이템1 리드 연속)인데 그 중간에 회수 경계가 앉았다. 비전 판정 자체는 옳다(앵커샷 맞음) — 문제는 "앵커샷 존재 ≠ 아이템 경계"인 구간에서 재스냅이 당길 컷이 없다는 것. ±20s dedupe를 어떻게 통과했는지는 로그 유실로 미규명 — 다음 이상 시 readActivityLog 한도를 늘려 회수 시각을 확보할 것(활동 로그는 패널 재부팅 시 소실, connectPanel은 sessionId 없으면 reload=false여도 재부팅한다).
- **교훈**: ①실기 결과와 라벨이 어긋나면 **라벨부터 의심** — §94·§95·§97에 이어 네 번째 결손 적발이고 이번엔 회수 경로가 적발자다. ②일요일 기획 회차는 무료 모델의 구조적 사각(단신 뭉침) — 회수가 실질 담당.
- 지속 검증 스코어보드(실기 비전 ON): 7/23 **100** · 7/24 **100** · 7/25 **100** · 6/28 **95.7**(FP 1).

## §104 회수 FP 기전 차단(edge 30) + 배치 구성 효과 확정 (2026-07-27)

지속 검증에서 나온 회수 FP 2건(6/28 27.25·6/03 35.0)의 공통 기전을 차단하고 재검증했다.

- **기전**: 두 FP 모두 프로브가 직전 확정 경계의 **앵커 리드 연장부**(26s 인사 테이크·25s 개표 스튜디오)에 떨어졌다. 비전 판정("앵커샷이다")은 옳았지만 아이템 경계가 아니고, 재스냅이 임의의 장면 전환 컷(6/03은 스튜디오→중계 전환)에 경계를 앉혔다. `edgeSeconds` 20이 20초 넘는 리드를 못 덮은 것.
- **처방**: edge 20→30(527b4ca). 길이 임계 방식은 기각 — 코퍼스에 진짜 16~21초 아이템이 15개 있어 재현율을 깎는다.
- **재검증**: 6/03 92.3→**100**(FP 소멸·회수 잔여 1건은 하단 띠 검사가 배제 — 방어층 협동 확인). 6/28 95.7→**95.2**(FP 소멸 ✓, 대신 841.0 날씨 단신이 이번 실행에서 FN — 후보 비결정성으로 834 후보가 안 나왔고 회수 842 프로브도 non-anchor 판정).
- **배치 구성 효과 — 반복 패턴으로 확정**: 842·845 프레임은 명백한 앵커샷(데스크·날씨 띠)인데 유실·임계 미달 없이 non-anchor로 판정됐다. §101-b의 799.63(진단 배치 3회 만장일치 0.98 vs E2E 배치 non-anchor)과 동일 — **footage가 다수인 회수 배치에 앵커 1~2장이 섞이면 판정이 non-anchor로 쏠린다.** 처방 후보(미실험): ①배치 셔플(공짜·효과 불확실) ②앵커 판정 프레임만 2차 재판정 ③프레임당 독립 판정(참조 반복으로 비용 5배 — 비권장). 다음 사이클 과제.
- **지속 검증 스코어보드(실기 비전 ON·edge 30 기준)**: 7/23 **100** · 7/24 **100** · 7/25 **100** · 6/03 **100** · 6/28 **95.2**(FN 1 — 비결정). 관측된 FP 기전은 전부 차단, 남은 변동은 후보 비결정성(§59)과 배치 구성 효과.

## §104-b 여유 비대칭(앞 30/뒤 20) 검증 + 잔여 한계 확정 (2026-07-27)

- **배치 구성 효과 기각(A/B 진단)**: 842 프레임을 footage 위주 배치와 앵커 위주 배치로 각 3회 판정 — **양쪽 모두 anchor 0.98~0.99 만장일치.** §101-b·§103에서 의심했던 "footage 다수 배치의 쏠림"은 원인이 아니다.
- **841 FN의 진짜 원인 = 뒤쪽 여유 절단**: edge 30 일괄 적용이 마지막 프로브를 838로 끊어 마지막 단신(841~872 = 31s)의 리드(841~848)가 격자에서 통째로 빠졌다. FP 2건은 전부 앞쪽 기전이므로 **앞 30/뒤 20 비대칭**으로 분리(f0e66e7).
- **최종 재실행(Sun4)**: 841.25 회수 ✓·FP 0. 다만 이번엔 768.25가 결손(직전 두 실행에선 검출) — TP 10·FP 0·FN 1 = **95.2**.
- **잔여 한계(정직 기록)**: 5s 격자 + 5~8s 단신 리드 = 리드당 프로브 0~1개이고, 그 1개가 **리드 종반**(768.25의 773 프로브 = +4.75s)에 걸리면 판정이 실행마다 흔들린다. 6/28 3회 실행이 각기 다른 1개를 놓친 이유(841→768). 개선 후보: step 4s(+25% 비용) 또는 non-anchor 프로브 사이 컷 존재 시 보간 프로브 — 비용 대비 효과는 다음 사이클에서 검토. 실사용 중심(평일 8뉴스)에서는 이 한계가 발현되지 않았다(7/23·7/24·7/25 전부 100).
- **오늘의 최종 스코어보드(실기 비전 ON)**: 7/23 **100** · 7/24 **100** · 7/25 **100** · 6/03 **100** · 6/28 **95.2**. 관측된 FP 기전 전부 차단(재현 실험으로 검증), 오배제 0 유지, 코퍼스 동일 회차 무하락.

## §105 3/06 실기 — 파이프라인이 라벨을 양방향으로 이김, 정정 후 F1 100 (2026-07-27)

경고 대상 회차 3/06(구 라벨 F1 96)의 실기가 오늘 세 번째 라벨 검증 사이클을 촉발했고, 이번엔 **양방향 모두** 파이프라인이 옳았다.

- **실행**: 경고(0.104) → 후보 14 → 비전 배제 4 → 회수 4 = 아이템 14. 구 라벨(13) 기준 TP 10·FP 4·FN 3 = 74로 보였다.
- **판독 결과 — 일곱 지점 전부 파이프라인 승**:
  - 회수 추가 4건(191·238.75·702.75·732.5) = 전부 **새 헤드라인의 앵커샷**(통합특별시 출범·장성 화재·목포해양대) — 236s 뭉친 아이템 등의 **라벨 결손**.
  - 비전 배제 3건(461.5·492·508) = 전부 **광주FC 코너 내부 footage·선수 인터뷰** — 라벨의 **허위 경계**, 배제가 정답(오배제 아님).
- **정정**: 삭제 3·신규 4 = **14아이템**(신규 4건 컷 전후 프레임 직접 재검증, 702.75는 0.25s 그리드로 좁힘). 정정 후 실기 재판정 **TP 14·FP 0·FN 0 = F1 100.**
- **의미**: §103(회수가 결손 적발)에 이어, 이번엔 **비전 배제가 허위 경계까지 적발**했다 — 검증·회수 두 방어층이 라벨 QA 도구로 기능한다. 코퍼스 라벨의 신뢰도 서열이 뒤집혔다: 정밀 판독 대상이 안 된 구회차 라벨은 실기 결과와 어긋날 때 우선 의심 대상이다.
- **오늘 최종 스코어보드(실기 비전 ON)**: 7/23 **100** · 7/24 **100** · 7/25 **100** · 6/03 **100** · **3/06 100** · 6/28 95.2. 전량 84회차 오프라인 96.9(정답 아이템 수 증가·무료 경로 한계 반영 — 실기와 별개 지표).

## §105-b step 4s 검증 — 6/28 F1 100, 오늘 실기 6회차 전원 100 (2026-07-27)

- **6/28 최종 실행(step 4)**: 후보 4 + 회수 7 = 11아이템, 정답 11과 전부 ±2.5s 안 1:1 — **TP 11·FP 0·FN 0 = F1 100.** 직전 실행들이 번갈아 놓치던 768.25(리드 종반 프로브)와 841.0(뒤 여유 절단→복원)을 둘 다 회수했다. 리드 시작 4초 안 프로브 보장(§105-b)이 기하학적으로 작동.
- **오늘의 최종 스코어보드(실기 비전 ON)**: 7/23 **100** · 7/24 **100** · 7/25 **100** · 6/03 **100** · 3/06 **100** · 6/28 **100**. 미학습 3회차 + 경고 회차 3회차(오경보 1·실패 2, 라벨 정정 포함) 전원 100.
- 하루 동안 회수 경로에서 걷어낸 결함 사슬(각각 실기 반복으로 검증): 배치 바이트 상한 침묵 실패 → 10s 격자의 짧은 리드 이탈 → 배경 인물 프롬프트 오독 → 같은 회차 참조 부재 → 배제 후보 재심 부재 → 앞 여유 부족(리드 연장부 FP) → 뒤 여유 과다(마지막 단신 절단) → 5s 격자의 리드 종반 흔들림. **여덟 겹이었고, 전부 실측이 가려냈다.**
- 남은 검증: 7/27 이후 신규 업로드 블라인드(라벨러 독립성 확보된 진짜 지속 시험), 리드 종반 개선의 타 회차 회귀(다음 신규 회차가 겸함), H-1 AI 큐 우회.

## §106 비전 경로 AI 큐 경유 — H-1 비용 보호 우회 해소 (2026-07-27)

릴리스 리뷰(§99)가 남긴 마지막 지적. 저장소의 다른 모든 OpenAI 경로는 `aiQueueController.run`을 거치는데 **비전 검증·회수만 큐 밖**이라, 일일 요청·비용 한도가 적용되지 않고 "오늘 사용" 카운터에도 잡히지 않았다. 옵트인 시절엔 사용자가 고른 예외였지만 기본 ON(§96) 이후로는 "비용 보호가 기본으로 꺼진 유료 기능"이었다.

- **처방**: `runVisionBatch()` 헬퍼로 검증·회수의 모든 배치 호출을 큐에 태운다. `estimateUnits`는 배치의 프레임 수(비용 비례), `maxRetries: 0`(이 경로는 유실 재판정·배치 격리로 자체 복원 — §92, 큐 재시도와 이중이 되면 비용이 배가된다), 큐 미초기화 시 그대로 실행(비용 보호는 못 하되 분할은 막지 않음).
- **`confirmRequired`는 넘기지 않는다(설계 판단)**: 승인 대기는 큐가 잡을 붙든 채 사용자 클릭을 기다린다. 분할은 이미 스캔·프레임 내보내기를 마친 뒤라 여기서 멈추면 **멎은 것처럼 보인다**(승인 UI는 다른 탭에 있다). 이 경로의 사전 동의는 ①AI 전송 동의 게이트(§99) ②"(유료)" 라벨·가시 비용 고지 ③실행 직전 전송량 활동 로그가 담당한다.
- **남는 한계(정직)**: ①요청 수는 큐 규약상 **배치당 1**로 잡혀 회차당 실제 API 호출 수보다 적게 집계된다(자막 AI 등 기존 다배치 경로와 동일 규약) ②배치 단위 승인은 미해결 — "긴 작업 중간에 멈추지 않으면서 큰 비용을 사전 승인받는" UX가 필요하고, 실행 **전** 예상 비용을 보여주고 한 번만 묻는 방식이 유력하나 별도 설계 과제다.
- **주의**: 큐 예산은 사용자가 AI 큐 탭에서 "저장"을 눌러야 설정된다(`setBudget`은 저장 버튼 핸들러에만 있다). 저장 전에는 한도가 `undefined`라 아무것도 막지 않는다 — 큐 경유의 실효는 사용자가 한도를 정한 뒤에 생긴다.
- 게이트 check 1838 그린. 실기 검증은 다음 신규 회차 블라인드가 겸한다(비전 경로 자체는 무변경, 호출 래핑만).

## §107 배치15 — 오프라인 결손 회차 실기 검증, FN 회수 확장 (2026-07-27)

사용자 제안("신규 업로드 대신 올해 분 미검증 회차로 테스트")에 따라 오프라인 F1<100인 32회차 중 8회차를 실기로 돌렸다.

| 회차 | 유형 | 오프라인 | 실기 | 비고 |
|---|---|---|---|---|
| 6/06 | FP형 | 90.9 | **100** | 비전이 FP 2 제거 |
| 7/14 | FP형 | 91.7 | **100** | 배제 3건 전부 옳음 |
| 6/10 | FP형 | 90.9 | **100** | 실기가 라벨 결손 적발(754 "여객기 참사" — 다섯 번째) → 정정 후 100 |
| 7/15 | FP형 | 88.9 | 94.1 | FP 1 잔존(252 — 비흰색 인용띠 + 비전 만장일치 실패, §92 잔여 유형 첫 실기 확인) |
| **6/23** | **FN형** | 96.3 | **92.3** | **FN 2 잔존 — 뱅크 정상이라 경고·회수 미발동(설계 구멍 실측)** |
| **6/27** | **FN형** | 94.1 | **87.5** | 동일 — FN 2 잔존 |
| 6/05 | — | 90.9 | 무효 | 2부작 회차의 다른 파일 실행(정답 끝 575.75 vs 938.75) — b파일 재실행 필요 |
| 7/12 | — | 90.0 | 미실행 | 채널에 1080 H.264 없음 → 720p 가드 정상 거부 — 업스케일 재실행 필요 |

- **핵심 발견 — 회수 발동 조건의 구멍**: FN형 회차는 뱅크가 정상(경고 없음)이라 회수가 아예 돌지 않는다. 6/23·6/27의 FN 4건 중 **3건이 100s+ 구간 안(회수 사정권)**이었다. → **§107 확장: 회수를 비전 ON 전체로**(경고 무관). 정상 회차 추가 비용은 리포트 길이 구간 훑기(프레임 수십 장), FP 위험은 §104 기전 차단·0.9 임계·하단 띠 협동으로 관리. 경고 자체는 유지(사용자 안내용).
- 잔여 FP 유형(252): 비흰색 인용띠 + 비전 4프로브 만장일치 실패 조합. 만장일치를 3/4로 낮추면 오배제 위험이 커지므로(834 사례) 손대지 않고 기록만 — 빈도가 쌓이면 재검토.
- 실기 검증 누계: **12회차 중 9회차 F1 100** (95.2 이상 12/12).

## §107-b 회수 전면 발동 원복 — 실측 순손실 확인 (2026-07-27)

§107(회수를 비전 ON 전체로 확장)을 배치15c로 검증한 결과 **원복이 정답**이었다.

- **실측 손익**: 이득 = 6/23의 FN 1건(223.5) 회수. 손실 = 비경고 회차에서 회수 FP 3건 — 7/12 354.75(footage B-roll 컷, 새 띠는 컷 2.5초 전 선행)·444.5(기자 스탠드업 시작 컷)·6/05b 525.5(**컷조차 없는** PIP 인터뷰 내부). 순손실이므로 경고 조건(bankFit>0.1)으로 원복.
- **대조**: 경고 회차의 회수는 §104 이후 FP 0을 유지한다. 차이의 해석 — 경고 회차의 긴 구간은 "뱅크가 놓친 진짜 앵커"가 실재하는 공간이고, 비경고 회차의 긴 구간은 대부분 진짜 긴 아이템이라 훑기가 오판 노출만 늘린다. 특히 PIP 인터뷰(화면 속 착석 인물)는 §101-b의 "배경 인물" 절이 커버하지 못하는 유형으로, anchor 0.9+ 오판이 가능함이 확인됐다.
- **6/27 라벨 오류 정정**: 475.75는 앵커샷 컷이 아니라 **인터뷰 자막 페이드인 완료 시점**을 경계로 오기입한 것(±6s 창 전체에 앵커샷 전무 — 서브에이전트 3단계 판독 + 본인 프레임 확인). 삭제, 8아이템.
- **원복 후 실기 스코어보드(14회차)**: 7/23·7/24·7/25·6/03·3/06·6/28·6/06·7/14·6/10·**7/12·6/05** 100(11회차) · 7/15 94.1(§92 잔여 인용띠 FP) · 6/23 92.3(대담 코너 known-FN 등) · 6/27 93.3(짧은 구간 FN — 회수 사정권 밖). 잔여 결손은 전부 유형이 규명된 known-issue다.
- **교훈**: "사정권에 FN이 있다"는 것과 "그 확장이 순이득"은 다르다 — 확장은 반드시 손익 실측으로 판정한다. §107은 커밋 후 4시간 만에 실측으로 원복됐고, 이것이 게이트 원칙이 작동하는 모습이다.

## §107-c PIP 오판 기전 조사 — 프롬프트·참조 모두 무죄, 보강 미반영 (2026-07-27)

§107-b의 회수 FP 3건(PIP 인터뷰·스탠드업·footage 컷) 기전을 A/B 진단 2종으로 조사했다.

- **A/B①(현행 vs 보강 프롬프트, 참조 없음)**: 오판 프레임 3장 + 대조 앵커 3장(6/05b 53·6/28 842·7/23 쇼벨 804)을 2회씩 판정 — **현행 프롬프트가 이미 6/6 정확**(PIP·스탠드업 non-anchor 0.98+, 앵커 anchor 0.97+). 보강 문구("PIP·인셋 창·데스크 부재는 non-anchor")를 넣어도 판정 변화 없음.
- **A/B②(참조 없음 vs 같은 회차 참조 3장 포함)**: 참조를 넣어도 6/6 정확 — §101-c 참조 주입이 판정을 앵커 쪽으로 끌었다는 가설도 기각.
- **결론**: 실기 회수의 FP 3건은 진단에서 재현되지 않는 **실기 고유의 산발 오판**(패널 export 프레임 차이 또는 API 비결정성). §107-b 원복으로 비경고 회차 회수가 없어져 이 경로는 이미 실전 차단됐고, 프롬프트 보강은 **개선이 실측되지 않아 미반영**(게이트 원칙 — 효과 없는 변경은 넣지 않는다). 경고 회차에서 같은 유형이 재발하면 그때 회수 판정에 2표 합의(같은 지점 2프레임)를 검토한다.

## §108 외부 방법 조사 + 자기 유사성 실험 (2026-07-27)

F1 100의 잔여 결손(인용띠 FP·대담 코너 FN·짧은 구간 FN·산발 비결정성)을 겨냥해 외부 접근을 조사하고 1건을 실측했다.

- **조사 결과 4갈래**: ①회차 내 반복 샷 클러스터링(고전 앵커샷 검출 원리 — "앵커샷은 유일하게 반복되는 샷") ②하단 띠 텍스트 변화 신호(GDELT 방송 OCR 사례·오버레이 텍스트 추출 연구 — 잔여 결손 3유형 전부 커버 가능, Tesseract는 한글 오버레이 품질 이슈라 기존 비전 인프라로 띠 크롭만 읽는 게 현실적) ③TransNetV2 샷 경계 NN(PySceneDetect 65.5% 대비 87% — 재스냅 정밀 보강용, 후순위) ④CLIP ONNX 로컬 제로샷(비전 API 대체 잠재력, UXP 배포 복잡도로 장기).
- **①을 즉시 실측 — 왼쪽 6열 자기 유사성**: 전체 luma로는 신호 0(합성 구도라 앵커끼리도 배경이 다름 — §100 그대로). **왼쪽 6열(의상+데스크)로 재면 신호가 살아난다** — §74-d의 기각 사유("의상이 매일 바뀜")가 같은 회차 안에서는 사라지기 때문. tau=0.08·먼 이웃(±25s 밖)≥3 기준: 7/23 리드 재현 **94%**(오탐 12.2%) · 7/22 재현 79%(오탐 21.2%).
- **판정**: 단독 검출기로는 정밀도 부족. 유망한 쓰임 두 가지 — ⑴**회수 프로브 사전 필터**: 비전에 보내기 전 자기 유사성 지점만 훑어 프로브를 ~1/8로 줄이면서 후보 94% 보존(비용 절감 또는 같은 비용으로 더 촘촘한 격자) ⑵경고 회차에서 verified가 부족할 때 **같은 회차 참조 후보 발굴**. 제품화는 다음 사이클(오프라인 전량 스윕 → 손익 실측 → §107-b 교훈대로 순이득일 때만).
- 실험: scratchpad selfsim2.mjs.

## §109 외부 방법 4종 밤샘 실측 — 띠 신호 채택, 3종 기각/보류 (2026-07-27)

§108에서 조사한 4갈래를 전부 실측했다(사용자 지시 — 순차·끝까지).

| 방법 | 실측 | 판정 |
|---|---|---|
| ① 회차 내 자기 유사성(왼쪽 6열) | 전량 84회차 리드 재현 평균 73.9%·오탐 19.3%, 회차 편차 31~97% | **기각** — 사전 필터로 쓰면 후보 26% 손실. 참조 발굴 보조로만 |
| ② **하단 띠 변화+안정 신호** | 8회차 재현 **96%(72/75)**·FP 회차당 9~19(비전이 거를 수준). 실기 FN이던 6/27 185.5 포착. **96×54 스캔 등가 해상도에서 신호 보존 = 추가 비용 0** | **채택 — 제품화 1순위** |
| ③ TransNetV2 컷 검출 | 7/23 정답 12경계 전부 0.5s 안, **11개 오차 0.00s**, 디졸브 58.7도 0.15s. 1002s 영상 CPU 262s | 정밀도 최고이나 파이썬+torch 의존 — UXP 배포 불가. **보류**(인점 프레임 정밀화 요구 시 사전처리 도구로) |
| ④ CLIP ViT-B-32 제로샷 | 앵커 재현 **25%**(6/24)·비앵커 100% — 합성 구도 앵커를 footage로 오독(마진 중앙 -0.014) | **기각** — GPT 비전(0.98)과 비교 불가. 대형 모델 미검증 상태로 종료 |

- **②의 원리**: 새 헤드라인 띠는 등장 후 수십 초 유지되고 인용띠·자막은 수 초 단위로 바뀐다 — "변화(diff>0.10) 후 8초 안정(diff<0.06)"이 새 아이템 신호. 놓친 3건 = 58.7(오프닝 디졸브 — 기본 후보가 잡음)·6/23 223.5·618.25(대담 코너).
- **②의 제품화 스케치(다음 사이클)**: 스캔 루프에서 96×54 프레임의 하단 띠 행(y44~50·x9~89) 벡터를 grid와 함께 보관 → 스캔 후 띠 이벤트 계산 → 비전 ON이면 이벤트 지점(회차당 25~30개) 중 기존 경계 ±8s 밖을 회수 프로브로 판정. §107 실패 원인(비경고 회차 무차별 훑기의 오판 노출)이 없는 재도전 — 지점이 1/6이고 전부 사전 신호가 있는 곳. 손익은 §107-b 교훈대로 실측 후 채택.
- 실험: scratchpad selfsim3.mjs·band-signal2.py·band-sim96.py·transnet-eval.py·clip-test.py(mlenv venv — torch 2.13 CPU).

## §110 하단 띠 이벤트 회수 프로브 제품화 (2026-07-28)

§109 채택안(②)을 제품화했다(커밋 4eb7a86). 스캔 BMP에서 `bandRow()`(80×6, x9/96~89/96·y44/54~50/54)를 grid와 함께 캐시하고, 스캔 후 `detectBandEvents()`(변화>0.10 → 4표본 연속 <0.06, 간격 10s)로 이벤트를 계산, 비전 ON이면 회수 프로브 3종을 합친다 — ①균등 격자(경고 회차만, §107-b 유지) ②띠 이벤트(전 회차, verified ±8s 밖·꼬리−5s 앞만) ③배제 재심(+1.2s). 유닛 4종(이벤트 검출·흔들림 기각·간격 중복 제거·null 스킵) 포함 1842 테스트 그린.

- **1차 실기 3연전(7/26 밤)**: 7/23X 12/12 · 7/24X 10/10(띠 프로브 16개 전원 무해 — FP 0 입증) · **6/27X 7/8(185.5 미회수)**. 회귀는 통과했으나 회수 목표가 실패 — §110-b로 이어짐.

## §110-b 6/27 미회수 원인 규명 — 한도 기각의 유실 위장 (2026-07-28)

185.5 미회수를 층별로 배제해 근본 원인을 확정했다. 각 층의 실측:

1. **띠 신호**: ffmpeg 96×54 등가 재연 — 이벤트 17개·8/8 적중(185.5는 **188.0s 이벤트**, 변화량 0.393). 실기 프로브 시각 로그에도 188.0 포함 — 신호·배관 무결.
2. **라벨**: 185.5 프레임 직독 — 좌측 데스크 앵커(초록 재킷)+참배 영상월 합성 구도, 진짜 앵커. 앵커 인트로가 ~6s로 극단 단문(189.5 띠 표시, 193.5 푸티지). 후보 생성이 놓친 것도 재확인(후보 8 = 476 포함, 185.5 없음 — 476 배제는 라벨 정정과 일치하는 옳은 배제).
3. **비전 판정**: 프로덕션 프롬프트·모델(gpt-5.4-mini·effort low)·Exporter 480px 바이트·참조 5장(같은 회차 3+코퍼스 2)·실기 배치 구성(푸티지 6장 동석)을 전부 재현 — **188.0 = 앵커 0.98~0.99, 10/10 결정적**. 1프레임 배치(참조 5+Frame 1)도 4/4 정상(인덱스 시프트 가설 기각).
4. **근본 원인**: 큐 상태 직독 — 이미지 잡 전원 `state:"failed"`, error **"오늘의 AI 비용 단위 한도를 초과합니다."** 기본 한도 100단위/일이 7/27 낮의 11회차 배터리로 소진 → 밤 3연전의 검증·회수 배치가 API에 닿기 전 기각 → catch가 삼켜 **"판정 유실"로 위장**(7/24X 검증 40장 유실 경고의 정체도 동일). 비전 ON 1회차 = 검증+회수 50~90단위라 **기본 100은 하루 1회차 만에 침묵 소진**되는 설계였다.

교훈: **"판정 유실"과 "한도 기각"은 다른 병이다** — 원인 필드를 삼키는 catch가 이번 주 최장 디버깅(신호→라벨→비전→큐 4층)을 만들었다. 유실 개수만 세지 말고 실패 사유를 유형별로 가시화할 것.

## §110-c 한도 처리 3종 시정 (2026-07-28)

1. **한도 기각의 명시 중단**(검증·회수 공통): catch에서 메시지에 "한도" 포함 시 남은 배치·재판정 라운드를 즉시 접고 별도 경고 — "AI 일일 한도 도달 — …중단합니다(미판정 N장). 설정 탭에서 한도를 조정할 수 있습니다." 더는 유실로 위장되지 않는다.
2. **기본 비용 한도 100→400단위**(ai-queue-controller): 비전 ON 3~4회차 여유. 저장된 예산이 있는 기존 패널은 restore가 저장값을 유지하므로 설정 UI에서 직접 조정(이 패널은 400 저장 후 야간 배터리용 1000).
3. **회수 판정 관측성**(§92 검증 경로 이식): 수신 인덱스 대조 → 유실분 1회 재판정 → "회수 프로브 시각:"·"회수 판정 상세:"(프레임별 앵커/비앵커·신뢰도)·유실 시각 로그. 이번 규명을 가능하게 한 로그를 상설화.

~~미해결(백로그)~~ → 시정 완료: 일일 사용량 날짜 키를 UTC에서 로컬 날짜로 변경(한국 09:00 리셋 결함 — 업그레이드 첫 로드에 사용량이 1회 리셋되는 부작용은 무해). 러너(cdt-blind-run) 감시 필터에 "회수|재심" 누락돼 §110 로그가 안 잡히던 것도 수정됨(스크래치패드).

## §110-d 시정 후 실기 3연전 — 전원 F1 100, §110 채택 확정 (2026-07-28)

§110-c 시정 빌드(한도 1000 임시 상향)로 3연전을 재실행했다. 판정은 시퀀스 in/out 직독(cdt-read-ranges) × 라벨 파일 대조.

| 회차 | 결과 | 세부 |
|---|---|---|
| 6/27X | **8/8 = F1 100** (87.5→100) | 프로브 188.0→앵커(0.99) 회수, 재스냅이 185.5에 **오차 0.00s** 안착. 나머지 12프로브 전원 비앵커 0.98+(FP 0) |
| 7/23X | 12/12 = F1 100 유지 | 경고 회차 회귀 무해(392.81 아이템도 0.19s) |
| 7/24X | 10/10 = F1 100 유지 | 띠 프로브 13~16개 전원 무해 — §107 실패 요인(무차별 훑기 오판 노출) 재발 없음 |

회수 판정 상세 로그가 설계 의도를 그대로 실증했다 — 띠 이벤트 프로브는 "사전 신호가 있는 지점"만 훑므로 비앵커 지점은 0.98+로 깨끗이 기각되고, 진짜 놓친 경계(6s 단문 인트로)만 0.99로 회수된다. §110 채택 확정. 잔여 결손은 6/23 대담 코너(223.5·618.25 — 띠 신호도 §109에서 놓침)와 7/15 인용띠 FP(252 — 검증 경로 별건)뿐.

## §111 외부 조사 2차 + 신규 신호 2종 오프라인 실측 (2026-07-28)

F1 100 견고화를 위한 외부 레퍼런스 조사(사용자 지시)와, 잔여 결손을 겨냥한 §109식 사전 실측.

- **문헌 정합**: ①TF1 뉴스 챕터링(arXiv 2406.17590) — 오디오+비주얼 frozen 특징 → LSTM 융합, 정밀도 82%@IoU90. 멀티모달 융합이 표준 다음 수순임을 확인. ②2025 비교 연구(arXiv 2503.21848) — 뉴스 분할에서 **단순 이미지 분류기(ResNet 84.3%)가 시간축 비디오 모델(ViViT)을 능가** — 우리 프레임 기반 GPT 비전 접근이 증거 우위 쪽. ③고전 다중모달 분할 문헌 공통 신호 = 앵커 얼굴·샷 경계·자막 캡션·**멈춤/화자 교체·오디오 이벤트** — 우리 미사용 신호는 오디오뿐.
- **배포 벡터**: TransNetV2 **ONNX 변환본 존재**(HF elya5/transnetv2, 입력 48×27×100). 단 UXP WASM은 Photoshop 2025 크래시 보고가 있어 취약 — §109 보류 판정 유지, 데스크톱 사전처리 도구 선택지로만.
- **잔여 결손의 정체 재규정(프레임 직독)**: 6/23 223.5·618.25는 대담 코너가 아니라 **"앵커 없이 푸티지로 시작하는 아이템"**(224.7=위성지도+좌상단 '단독' 띠, 619.5=국감장 와이드+'KBC 대담' 띠). 앵커샷이 원리적으로 없으므로 앵커 기반 후보·검증·§110 하단 띠 전부 사각. 포착하려면 "아이템 시작 = 앵커 복귀 **또는** 새 코너 그래픽"으로 회수 판정 의미 확장이 필요하다.
- **신규 신호 실측 A — 좌상단 띠 이벤트**(crop 21:3:4:4, §110 동일 임계): 6/23 **618.25 포착(+3.75s)**·FP 6, 6/27 FP 4, 7/23 FP 8 — 프로브 소스 규모로 무난. **223.5는 미발화** — 63픽셀 크롭에서 배지 내 텍스트 교체가 diff<0.10(해상도 천장).
- **신규 신호 실측 B — 오디오 novelty**(Foote SSM, 8kHz PCM·순수 numpy — JS 이식 가능 수준): 6/23 **618.25 포착(±5.75s)**, 223.5 미포착, 피크 25/회차. 패널에 WAV 내보내기+parseWavPcm이 이미 있어 제품화 가능 경로.
- **권고 우선순위**: ①좌상단 띠 이벤트 + 회수 프롬프트 의미 확장(§111-a 후보 — 618류 즉효, 회귀 배터리 필수) ②오디오 novelty는 코퍼스 전량 스윕 후 띠와 합집합 검토 ③223.5류(초소형 배지 교체)는 해상도 천장 — 고해상 배지 크롭 별도 내보내기(비용 증가)와 교환 관계라 이번 사이클 보류·문서화 ④7/15 인용띠 FP 252는 §82 필터 일반화(비백색 인용띠)로 별건 처리.
- 실험: scratchpad audio-novelty-623.py·band-sim-top.py. 진짜 완료 조건(신규 업로드 블라인드 3연속)은 오늘 방송분부터.

## §111-a 좌상단 띠 회수 실기 배터리 — 원복 판정, 라벨 오류 2건 적발 (2026-07-28)

권고 1순위를 제품화(TOP_BAND_REGION·classifyItemStarts·회수 2풀)해 실기 3연전을 돌렸다. 판정은 원복이지만, 수확이 판정보다 크다.

- **6/23X**: 회수 4건 추가 중 프레임 직독 결과 — ①**204 회수는 옳았고 라벨(223.5)이 틀렸다**(202.0 인용 카드 등장이 진짜 시작 — 정정 반영). ②**570 회수(하단 띠 프로브→앵커 0.98)도 옳았다** — 570은 KBC 대담 스튜디오 세트. 그런데 **"앵커 16개" 병합 후 최종 14개에서 570·346이 소멸** — 재스냅·병합 단계가 회수 경계를 삼키는 신규 결함(§111-b 백로그, 526.75와의 20s 병합 흡수 의심). ③256은 같은 스토리 중간 지도샷 — **아이템 판정의 실제 오판 모드**("새로 등장한" 그래픽인지 단일 프레임으로 판별 불가).
- **6/28X**: 후보 4개로 붕괴(뱅크 미인식)→경고→격자 168프로브→F1 72. §111-a와 무관한 별개 요인 — 이전 실기 100과 소스·시퀀스 차이 재현 조사 필요(§111-c 백로그).
- **7/24X**: §73-d 가장자리 가드로 불발 — native1080.mp4가 필러박스 의심(이전 검증은 1080up 파일이었다).
- **판정**: 정확 라벨 기준 6/23 손익 +1(204)/−1(256) 무승부 + FP 모드 미해결 + 회귀 2회차 오염 — §107-b 원칙대로 **코드 원복**(게이트 그린 확인). 좌상단 띠의 실가치(202류 인용 카드 시작)는 재스냅 소실(§111-b)을 고친 뒤 쌍프레임 문맥 판정(프로브±전 프레임)으로 재도전할 것.
- **라벨**: 6/23 223.5→202.0 정정(.corrected). **618.25는 이동 보류** — 570 대담 시작 실증과 526.75 병합 가능성이 얽혀 대담 코너의 아이템 시맨틱(코너 전체 1아이템 vs 코너 내 분할)을 정한 뒤 정정한다.
- 2순위(오디오 novelty)는 착수하지 않는다 — 표적이던 618/570형이 "§110이 이미 잡되 재스냅이 삼키는" 사례로 재규정됐으므로, §111-b 수정이 먼저다.

## §111-b 규명 — "재스냅 소실"은 결함이 아니라 자기 정리, 6/23 대담 시맨틱 확정 (2026-07-28)

§111-a에서 "회수 570·346이 병합 후 소멸"을 결함 후보로 백로그에 올렸으나, 프레임 브래키팅으로 반전 규명했다.

- **대담 시맨틱 확정**: 527.9·545.0 프레임 모두 이미 대담 2박스(진행자+송미령 장관) — **KBC 대담 코너는 526.75에서 시작**하며 후보 528로 이미 정확 검출·라벨돼 있었다. 코너 내부(스튜디오 대담·클로즈업·국감 자료화면 618~·와이드 570)는 전부 **한 아이템**. 라벨 618.25는 코너 중간 자료화면이라 경계가 아님 → 삭제.
- **§111-b 재규정**: 570·346은 아이템 중간의 중복 회수였고, `refineBoundaryToTransition`의 후방 확장(최대 36s)이 이들을 직전 경계의 컷까지 되돌린 뒤 1초 미만 필터가 흡수했다 — **"아이템 중간에 잘못 추가된 회수 경계의 자기 정리"라는 숨은 안전망**. 코드 수정 불요. 한계도 명확: 중간 FP가 진짜 컷 위에 앉으면(256 — 인용 카드→지도 전환 컷) 자기 정리가 못 잡는다 — 회수 추가의 FP 방어는 여전히 판정 품질이 담당한다.
- **6/23 라벨 최종 13아이템**: [54.25, 181.25, **202**(223.5 정정 — 인용 카드 시작), 287.5, 396.5, 526.75, 659.5, 694.75, 736.25, 779, 813, 855.25, 888.25]. **주의 — 정정 사고 1건**: 첫 정정 스크립트가 `.items.json`(15아이템)을 소스로 읽어 **이전 세션이 삭제했던 458을 부활시켜 커밋**(498a10b)했다 — 본 커밋에서 재삭제. 교훈: corrected가 존재하면 라벨 편집의 소스는 반드시 corrected다.
- 현행 배포 빌드 기준 6/23 재채점: 검출 12/13 · FP 0 → **F1 96.0**(구 92.3은 라벨 오류 포함 채점). 잔여 FN 1 = 202(앵커 없는 인용 카드 시작 — §111-a 기각 유형, 쌍프레임 문맥 판정으로 재도전 예정).

## §112 정밀도 로드맵 4단계 — 미정정 라벨 감사·시맨틱 규약·측정 위생 (2026-07-28)

회고(§110-b~§111-b)의 힌트를 실행했다. 순서와 결과.

### ⓪ 미정정 회차 라벨 감사 — "미정정 = 부실" 가설은 기각, 진짜 결손 지도 확보

오프라인 전량 평가로 F1<100 27회차(FP 33·FN 29)를 뽑고, 그중 미정정 13회차(7/23은 실기 100으로 라벨 기검증이라 제외 → 12회차)의 불일치 17건을 §95 경로(480p 재다운로드 + **길이 규칙 신원 검증**)로 전수 프레임 판독했다.

- **라벨 오류는 1건뿐**: 5/14 856 아이템 누락(앵커+기자회견 합성+새 띠 — 853 박수 푸티지와 컷 확인) → 정정본 생성(16아이템). 6/23(3결함)은 특이 회차였고 나머지 미정정 라벨은 견고 — **§95·§64의 정정 작업이 이미 수확을 끝낸 상태**임을 확인.
- **나머지 16건은 전부 시스템 몫**: 경성 FP 12(스탠드업 4·연단/무대 발언 3·리포트 중간 b-roll/CG/인터뷰 4·오프닝 예고 1) + 시스템 FN 2(5/19 683.5 합성 구도 앵커·6/30 651 **스탠딩 앵커** — 뱅크가 모르는 신규 유형 발견) + 6/05 판독 보류.
- **소스 오식별 2차 발견**: sources.json의 6/05 ID가 모닝와이드였다(§95 신원 검증 누락분) — 제거. 판독 전 길이 규칙 검증을 표준 절차로: 이번에 10편 중 1편을 걸러냈다.
- 코퍼스 재평가: **84회차 F1 96.8**(TP 934·FP 32·FN 29). 이 수치의 잔여 결손은 이제 라벨 노이즈가 아니라 **판독 확인된 시스템 결손**이다 — 경성 FP는 실기 비전 검증(§92)이 거르는 유형이 대부분이고, FN 2유형(합성·스탠딩)은 실기 §110 회수의 사정권.

### ① 아이템 시작 시맨틱 규약 — docs/NEWS_ITEM_SEMANTICS.md

시작 3유형(앵커 복귀/인용 카드/코너 그래픽 직행)·비시작 5유형(중간 그래픽·대담 내부·스탠드업·예고·아웃트로)·판독 규칙(FN은 이웃 라벨 필수, corrected 소스 원칙, ±0.75s 정밀화 창)을 명문화. 라벨·프롬프트·검증이 이 문서 하나를 공유한다.

### ② 측정 위생 2종

- **배터리 픽스처 명부**(scripts/host-smoke/field-fixtures.json): 회차→파일→시퀀스 고정 + 함정 주석(7/24는 1080up 필수·6/28 후보 붕괴 주의·6/05 b파일). §111-a의 회귀 오염 2건 재발 방지.
- **침묵 실패 유형화**(index.ts): 검증·회수의 판정 유실 경고에 원인 내역(응답 누락/타임아웃/레이트리밋/인증/기타) 부기 — §110-b(한도 위장 최장 디버깅)의 재발 방지. 게이트 그린.

### ③ 202류(앵커 없는 시작) 재도전 — 데이터 근거로 보류

감사 취지대로 빈도를 재평가했다: 판독 17건 중 202류(인용 카드/코너 그래픽 직행)는 **0건**, 전 코퍼스 기지 사례도 6/23 202 하나뿐. 희귀 유형에 상시 검출기를 붙이면 §111-a 실측처럼 FP 노출이 이득을 초과한다 — **쌍프레임 재도전은 이 유형이 2례 이상 재관측될 때까지 보류**(알려진 한계로 문서화, NEWS_ITEM_SEMANTICS.md). 대신 이번에 발견된 **스탠딩 앵커**(6/30 651)는 실기 §110 회수 사정권 검증이 다음 후보다.

## §112-b 쌍프레임 아이템 시작 판정 재도전 + §112-c 6/28 정체 완전 규명 (2026-07-28 오전)

2시간 자율 창의 작업. §112에서 보류했던 좌상단 띠 재도전의 조건(2례 이상)이 충족됐다 — 6/30 스탠딩 앵커 코너(651)가 하단 띠 사정권 밖(574→818 공백)이면서 **좌상단 띠 656 적중**으로 2례째가 됐기 때문.

- **§112-b 구현**: classifyItemStarts를 쌍프레임(Before t−6s + Frame t)으로 재설계 — §111-a의 실측 오판 모드("새로 등장한 그래픽인지 단일 프레임으로 판별 불가", 6/23 256)를 "Before에 없던 새 보도의 개시" 질문으로 차단. 스탠딩 프레젠터를 (a)절에 명시 포함. 회수 아이템 풀은 쌍 내보내기·쌍 청킹(12−참조를 2로 나눔)·유실 재판정·한도 공유로 §110-b 규약 동일. 게이트 그린.
- **실기 검증**: 6/23X — 쌍 프로브 7개 전원 정확 기각(FP 0), 7/24X(FriY) — 9개 전원 무해·10/10 유지. **오판 모드 해결 입증**. 단 진표적 204는 2연속 미승인(0.9 임계 대비 판정 요동) — 회수 실적 0. 순손실 0·비용 소액·표적 유형의 유일 경로라 **채택 유지**, 임계 하향(0.85)은 블라인드 데이터 축적 후 검토.
- **6/23X에서 §92 요동 재관측**: 이번 실행은 224 후보의 비전 배제가 발동하지 않아 F1 91.7(FP 224·FN 202) — 배제/유지가 실행마다 갈리는 전면 인용카드 경계 유형. 4프로브 규칙의 224 사례는 §92 산발 케이스로 기록.
- **§112-c — 6/28 붕괴의 완전 규명**: ①캐시·파일 신원 정상(길이 규칙 전 회차 검증) ②코덱 정상 ③일요일 fit 지형에서 0.100으로 **경고 임계(>0.1)의 칼끝** ④전면 합성 배경 회차라 무료 후보가 4개로 붕괴(§100 극단 사례) ⑤경고 발동 시에도 격자 회수의 비전 판정이 실행마다 요동(이번 77.8 vs 과거 100 vs §111-a 72). 결론: 6/28류(전면 합성)는 "경고+검토" 상태가 상용 현실 — 픽스처에 명시. 부수: 7/12 파일이 실제 720p임을 발견(명부 기록), 6/30 480p는 §73-d 가드로 실기 불가(스탠딩 실기 검증은 원본 재확보 후).
- **픽스처 함정 신종**: 시퀀스 정리 후 §111-a가 native(필러박스)로 FriX를 재생성 → 이후 실행들이 "already" 재사용으로 연쇄 가드 발동. 대응: FriY로 재생성. 교훈 — **시퀀스 재사용 전 클립 출처 확인, 오염 시 새 이름**.

## §113 신규 회차 블라인드(7/26 일) — 첫 진짜 블라인드, 대담 게스트 FP 신규 유형 발견 (2026-07-28)

코퍼스에 없는 7/26(일) 8뉴스 풀영상(919s·1080p H.264, ID bu-idpxt7P8)으로 **첫 블라인드**를 실행했다. 프로토콜: 파이프라인을 먼저 돌리고, 출력을 보기 전 컨택트 시트(4s 간격 230프레임)로 독립 라벨을 확정한 뒤 대조. 불일치는 전건 프레임 재판독으로 어느 쪽이 틀렸는지까지 갈랐다.

**정답 11개(프레임 검증 확정)**: 10.0 · 179.5 · 296.0 · 342.0 · 482.0 · 518.0 · 556.0 · 651.25 · 687.75 · 814.0 · 859.5 (끝 900).
독립 판독 초안에서 **296.0을 내가 놓쳤다** — 대담 코너 내부로 봤으나 프레임 확인 결과 앵커 단독 리포트 소개샷이었다(파이프라인이 맞고 라벨이 틀린 4번째 사례).

| 실행 | 조건 | TP/FP/FN | 정밀도·재현율·**F1** |
|---|---|---|---|
| A | AI 일일 한도 소진(검증 36장 중 **34장 미판정**·회수 3장 미판정) | 9/0/2 | 100 · 81.8 · **90.0** |
| B | 한도 해제(사용량 리셋·10만 단위) | 11/**3**/0 | 78.6 · 100 · **88.0** |

- **A의 결손 2건은 예산 고갈이 원인**이었다. 862s 프로브가 859.5s의 ±8s 안에 있었는데 한도로 판정 자체를 못 받았다. 사후 진단에서 862·484·861 프레임을 프로덕션 프롬프트로 3회씩 물으니 **전부 앵커 0.96~0.99** — 비전 능력 문제가 아님을 확정. 예산만 있으면 회수되는 구조였고 B에서 실제로 둘 다 회수됐다(재현율 100).
- **B의 신규 FP 3건 = 대담 게스트 단독샷**(94·130·269.75s). 세 프레임 모두 김민석 당대표 후보(출연자)가 스튜디오 데스크에 앉아 하단 띠를 단 구도로, 회수 비전이 앵커 0.97로 판정했다. 프롬프트의 앵커 정의("스튜디오 데스크에 앉아 카메라를 향한 진행자")를 **출연자가 그대로 만족**시키는 것이 원인 — 코퍼스에 대담 코너가 드물어 지금까지 노출되지 않았던 유형이다.
- **판정**: 완료 조건(신규 3연속 F1 100)의 카운트는 시작하지 못했다. 다만 A/B 대비로 결손의 성격이 갈렸다 — **재현율은 예산만 정상이면 100**이고, 남은 문제는 **대담 게스트 FP 하나**로 좁혀졌다.
- **다음 수정 후보**: 회수·검증 프롬프트에 "인터뷰/대담 출연자(게스트)는 진행자가 아니다 — 이름·직함 자막이 붙은 착석 인물, 진행자와 마주 앉은 구도는 아이템 시작이 아니다" 절 추가. §92 오배제 0 회귀 배터리 필수(진짜 앵커를 죽이면 안 된다).
- **운영**: 테스트 패널의 일일 한도를 요청 10만·비용 10만 단위로 해제하고 당일 사용량(300회·364단위)을 리셋했다. 실제 상한은 이제 OpenAI 잔액($2.38)뿐이다.

## §113-a 대담 게스트 절 — 7/26 블라인드 F1 100, 7/23 회귀 무해 (2026-07-28)

§113의 FP 3건(대담 게스트)을 겨냥해 프롬프트를 고쳤다. 진단이 원인을 한 번 뒤집었다.

- **오진 정정**: 처음엔 `classifyAnchorShots`를 고쳤으나 A/B에서 **구 프롬프트도 게스트를 0/9로 정확히 기각**했다(참조 주입 포함 24/24 정확). 로그 재판독 결과 FP 3건(94·130·270)은 앵커 판정이 아니라 **↑상단 띠 = 쌍프레임 `classifyItemStarts`** 경로였다. 세 쌍 모두 **Before=현장 푸티지 → Frame=스튜디오 착석 인물**이라, "푸티지에서 스튜디오 복귀 = 새 아이템"이라는 그 프롬프트의 정의를 게스트가 그대로 만족시킨 것 — 모델 오판이 아니라 질문의 결손이었다.
- **수정**: 게스트 절을 쌍프레임 프롬프트에 넣었다(자막이 기사 헤드라인이 아니라 인물 이름·직함이면 false, 대담은 같은 게스트로 계속 되돌아온다). 앵커 프롬프트에도 같은 절을 유지(A/B 무해 — 오배제 0/15 불변). 쌍 A/B: **FP성 오판 9/12→6/12 · 진표적 누락 3/6→0/6**.
- **실기 배터리**:

| 회차 | 결과 |
|---|---|
| **7/26 블라인드 3차** | 11/11 · FP 0 · FN 0 → **F1 100** |
| 7/23 회귀(대담 없음) | 12/12 · FP 0 → **F1 100 유지** |

- **주목**: 회수는 여전히 94·270을 추가했으나(쌍 판정 잔여 오판), **재스냅·병합의 자기 정리(§111-b)가 둘 다 흡수**해 최종 아이템에는 남지 않았다. 즉 이번 100은 "쌍 판정이 완벽해서"가 아니라 **판정 부분 개선 + 자기 정리 안전망의 합작**이다 — 안전망 의존이라 다음 회차에서 같은 조합이 항상 성립한다고 볼 수 없다.
- **완료 조건 카운트 1/3**(미학습 신규 회차 연속 F1 100). 7/26은 코퍼스 밖 진짜 블라인드였고 정답 11개는 전건 프레임 검증본이다.
- 잔여 과제: 쌍 판정의 게스트 오판 6/12를 더 줄일지, 아니면 아이템 시작 풀을 원복할지(실기 누적 진짜 회수 0건). 자기 정리에 기대는 현 상태는 취약하다.

## §113-b 아이템 시작 풀 원복 — 더 깨끗한 F1 100 (2026-07-28)

§112-b 아이템 시작 풀(좌상단 띠 + 쌍프레임 `classifyItemStarts`)을 전량 원복했다. 근거는 실기 누적 성적이다.

| 실기 | 쌍 프로브 |
|---|---|
| 6/23X·7/24X (§112-b 1차) | 진짜 회수 0 · FP 0 |
| 7/26 2차 | 진짜 회수 0 · **FP 3** |
| 7/26 3차(게스트 절) | 진짜 회수 0 · **FP 2**(자기 정리가 흡수) |
| 7/23 회귀 | 진짜 회수 0 · 판정 유실 2 |

**누적 진짜 회수 0건 · FP 5건.** 결정타는 7/26 3차의 내막이었다 — 무료 후보 11개가 이미 정답 11개를 전부 덮고 있었고, 쌍 풀은 FP만 더했으며 최종 100은 재스냅 자기 정리(§111-b)가 덮어준 결과였다. 표적(앵커 없이 시작하는 아이템)은 전 코퍼스 1~2건뿐이라(§112 감사 17건 중 0건) 상시 검출기의 FP 비용을 정당화하지 못한다.

- **제거**: `classifyItemStarts`·`ITEM_START_SCHEMA`·`TOP_BAND_REGION`·topBand 캐시/이벤트·쌍 프로브 내보내기·아이템 풀 판정 블록·관련 테스트(내가 만든 코드만).
- **유지**: 하단 띠 회수(§110), 게스트 절(앵커 프롬프트 — A/B 오배제 0/15 무해·272류 산발 방어), 3표 재판정(§113), bandRow의 BandRegion 일반화.
- **원복 후 실기**:

| 회차 | 결과 |
|---|---|
| 7/26 블라인드 | 11/11 · FP 0 → **F1 100**, **회수 추가 0건**(안전망 의존 없는 깨끗한 100) |
| 6/27 회귀 | 8/8 → **F1 100**, 하단 띠가 185.5 정상 회수 |

교훈: **"사정권에 표적이 있다"는 채택 근거가 못 된다**(§107-b 재확인, 세 번째). 표적 빈도와 FP 비용을 함께 재고 나서 결정할 것. 202·651류는 알려진 한계로 유지(NEWS_ITEM_SEMANTICS.md).

## §114 완료 조건 충족 — 미학습 신규 회차 3연속 실기 F1 100 (2026-07-28)

CLAUDE.md가 정한 완료 조건 3가지를 모두 만족했다. 세 회차 전부 **코퍼스에 없는 회차**이고, 실기 비전 ON에서 원클릭 분할을 완주했으며, 정답은 **파이프라인 출력을 보기 전에 컨택트 시트(4s 간격)로 독립 확정**한 뒤 불일치를 프레임 재판독으로 가른 것이다.

| 회차 | 정답 | TP/FP/FN | **F1** | 비고 |
|---|---|---|---|---|
| 7/26(일) | 11 | 11/0/0 | **100** | 대담 코너 회차·회수 개입 0건 |
| 5/20(수) | 11 | 11/0/0 | **100** | 752 경계를 §110 하단 띠가 회수 |
| 5/27(수) | 14 | 14/0/0 | **100** | 후보 18 → 비전이 FP 4건 정확 배제 |

- **조건 ① 신규 3연속 100** — 충족(위 표).
- **조건 ② 코퍼스 전량 무하락** — 충족. 오프라인 84회차 F1 96.8 유지, `check:news` 그린(원복은 오프라인 경로를 건드리지 않는다).
- **조건 ③ 비전 오배제 0** — 충족. 4회 실기에서 배제 5건(5/27 124·550·564·710, 6/27 476) 전부 프레임 확인상 비앵커였고, 진짜 앵커가 배제된 사례는 없다. 배제가 5/27의 정밀도 100을 만든 직접 요인이다.

**정직한 한정**: 세 회차는 "코퍼스 미보유"이지 "미래 회차"는 아니다. 5/20·5/27은 앞뒤 날짜(5/19·5/21·5/26·5/29)가 학습에 들어간 시기의 회차이고, 7/26만 코퍼스 최종일(7/25) 이후다. 더 엄격한 변형인 **당일 방송 직후 업로드 블라인드**는 별도로 남는다. 또한 판정 정답은 4s 격자 판독 기반이라 ±4s 오차를 포함한다(±8s 허용 기준에는 영향 없음).

**이 상태를 만든 요소**(오늘 하루):
- §110 하단 띠 이벤트 회수 — 5/20 752·6/27 185.5 회수 실적.
- §110-c 한도 기각의 명시화 + 기본 한도 상향 — 예산 고갈이 결손으로 위장되던 경로 제거.
- §113 3표 재판정 — 배제 요동 억제.
- §113-b 아이템 시작 풀 원복 — FP 원천 제거, 안전망 의존 해소.
- §112 시맨틱 규약·픽스처 명부·유실 원인 유형화 — 측정 자체의 신뢰도 확보.

## §115 블라인드 4·5회차 — 3연속은 유지, 5연속은 실패 (2026-07-28)

§114의 3연속 이후 6/01·6/04를 같은 프로토콜로 추가 측정했다(코퍼스 미보유, 실기 비전 ON, 라벨은 서브에이전트 1차 판독 후 본인 프레임 재검증).

| 회차 | 정답 | TP/FP/FN | **F1** |
|---|---|---|---|
| 6/01(월) | 10 | 10/**1**/0 | **95.2** |
| 6/04(목) | 8 | 8/**1**/0 | **94.1** |

- **재현율은 두 회차 모두 100** — 앵커샷은 하나도 놓치지 않았다. 6/01에서는 §110 회수가 340·840을 정확히 되찾아 336·836을 완성했다.
- **FP는 각 1건, 유형이 같다** — 6/01 542(3자 협약식 기념촬영: 정장 3인 정면+헤드라인 띠), 6/04 822(개표방송 스튜디오: 패널 3인 착석+헤드라인 띠). 둘 다 **"정장·정면·헤드라인 띠" 조합**이 앵커 정의를 만족시킨 오판이고, 발생 경로는 각각 회수 추가(542)와 검증 미배제(822)로 달랐다.
- **정정**: §114의 "완료 조건 충족"은 사실이지만 **안정적 100은 아니다**. 블라인드 5회차 기준 실측 성적은 **3/5가 F1 100**, 나머지 2회차가 94~95다. 상용 기대치를 말할 때는 이 5회차 표본(평균 F1 97.9·재현율 100·정밀도 96)을 쓸 것.

### §115-a 단일 진행자 절 — 채택

FP 3종(협약식·개표방송·대담 게스트) + 진짜 앵커 5종으로 3회 반복 A/B.

| 프롬프트 | 총 오판 | 개표방송 | 협약식 | 대담 게스트 | **오배제** |
|---|---|---|---|---|---|
| 현행(게스트 절) | 3/24 | 1/3 | 0/3 | 2/3 | **0/15** |
| **+단일 진행자 절** | **2/24** | **0/3** | 0/3 | 2/3 | **0/15** |

"앵커샷의 전경 주체는 항상 진행자 **한 명**이다 — 기념촬영·나란히 앉은 패널·마주 앉은 인터뷰 구도는 헤드라인 띠가 있어도 앵커가 아니다"를 추가했다. 개표방송 유형이 완전히 해결됐고 오배제는 불변이라 채택. 잔여는 대담 게스트 2/3(같은 프레임이 배치 구성에 따라 흔들림 — §103식 비결정성)이며, 실기에서는 이 유형이 회수 임계 0.9와 재스냅 자기 정리로 부분 상쇄된다.

## §116 학습 레버 2종 실측 — 둘 다 기각 (2026-07-28)

블라인드 FP(정장·정면·헤드라인 띠 유형)를 무료 경로에서 잡을 수 있는지 두 갈래를 실측했다. **둘 다 기각**이며, 근거를 남겨 재시도를 막는다.

### ① 모델에 띠 특징 추가 — +0.7, 불채택

현행 모델 특징은 144 luma 격자 + 전후 diff 2 + 뱅크 거리 1 = 147차원이고, **§110에서 96% 재현율로 입증된 하단 띠 신호가 들어 있지 않다.** 스캔 캐시에 띠 벡터가 없으므로 16×9 격자의 하단 2행(32셀)에서 파생한 근사 특징 5종(직전·직후 변화, 이후 4표본 안정도, 평균 휘도, 변화×안정 결합)을 추가해 같은 스윕으로 비교했다.

| | 홀드아웃 P | R | **F1** |
|---|---|---|---|
| 147차원(베이스라인 재현) | 89 | 69 | 77.9 |
| 152차원(띠 특징 5종) | 83 | 74 | **78.6** |

재현율은 +5 올랐으나 정밀도가 −6 내려 순증 +0.7에 그쳤고, 커밋 모델의 홀드아웃(F1 85.5)에는 한참 못 미친다. **원인은 해상도**다 — §110 띠는 80×6=480셀의 좁은 띠인데 격자 하단 2행은 화면 아래 1/3을 32셀로 뭉갠 것이라 텍스트 교체 신호가 남지 않는다. 진짜 띠 벡터를 학습에 쓰려면 84회차 스캔 캐시를 띠 포함으로 재생성해야 하고(회차당 15분·총 20시간), 그 비용 대비 기대 이득이 +0.7 수준이라 **불채택**.

### ② 앵커 위치 자기 일관성 필터 — 코퍼스 전량에서 기각

가설: 같은 회차의 진짜 앵커끼리는 좌측 인물 영역(16×9 격자의 c1~6·r2~8)이 서로 닮고, FP(협약식 기념촬영·개표방송 패널·대담 게스트)는 닮지 않는다. 진짜 앵커들의 셀별 중앙값을 중심으로 잡고 거리로 가른다.

- **블라인드 3회차에서는 완벽히 분리**됐다 — 6/01 542(0.189) · 6/04 822(0.156) · 7/26 94·130·270(0.155) 전부가 그 회차 진짜 앵커 최대 거리(0.085~0.124)를 넘었다.
- **그러나 코퍼스 83회차·진짜 앵커 724개로 넓히면 분리가 사라진다**: 진짜 앵커 최대 거리의 중앙값 0.118 · 90%tile **0.186** · 최댓값 0.387. FP들이 0.155~0.189이므로 진짜 앵커 분포의 꼬리와 완전히 겹친다. 절대 임계 0.15면 13/83 회차에서, 0.18이어도 9/83에서 진짜 앵커를 죽인다.
- 상대 규칙(중앙 대비 배율)도 같다 — FP 배율 2.6~3.5 vs 진짜 앵커 배율 90%tile 3.6.
- 3회차에서 좋아 보인 이유는 **그 회차들의 앵커가 유난히 균일**했기 때문(최대 0.085~0.124는 코퍼스 하위권). §108 자기 유사성 기각(재현율 73.9%)과 같은 벽이다.

**결론**: 이 FP 유형은 무료 luma 신호로 가를 수 없다. 판별 정보가 픽셀에는 있으나 격자 표현에는 없다는 §100 결론이 여기서도 유효하다 — 대응은 비전 프롬프트(§115-a 단일 진행자 절)와 회수 임계로 계속한다.

## §117 단일 진행자 절 실기 검증 + out-of-sample 확인 (2026-07-28)

§115-a에서 채택한 단일 진행자 절이 실기에서 FP를 실제로 지우는지, 그리고 그 효과가 **수정 설계에 쓰이지 않은 회차**로 일반화되는지 확인했다.
...
| 5/20(수) | 진짜 블라인드 | 100 | — |
| 5/27(수) | 진짜 블라인드 | 100 | — |
| 6/01(월) | in-sample(FP 542로 절 설계) | 95.2 | **100** |
| 6/04(목) | in-sample(FP 822로 절 설계) | 94.1 | **100** |
| **5/11(월)** | **out-of-sample(설계에 미사용)** | — | **100** (14/14) |

- 6/01 542(협약식 기념촬영)·6/04 822(개표방송 패널) 두 FP가 정확히 소멸했고 **재현율은 100 유지**(오배제 유발 없음).
- **5/11이 결정적**이다 — 이 회차는 절 설계에 전혀 쓰이지 않았고, 현재 빌드로 14아이템 전부를 잡고 과검출 0이었다. 절이 특정 회차에 과적합된 것이 아님을 보인다. 라벨은 서브에이전트 1차 판독(1초 스트립 재검증 포함) 후 본인이 4표본 프레임 확인.
- 서브에이전트가 유용한 함정 하나를 잡았다 — **컨택트 시트 라벨이 실제 영상 시각보다 1~3초 늦게 표시**된다(fps 필터 타임스탬프 특성). 라벨 확정은 시트가 아니라 `-ss` 직접 추출값으로 할 것.
- 현 시점 블라인드 누적: **6회차 전원 F1 100**(그중 4회차가 현재 빌드 기준 out-of-sample). 다만 6/01·6/04의 100은 in-sample임을 표기해 둔다.

## §118 코퍼스 90회차 확장 + 재학습 시도 — 모델 미개선 (2026-07-28)

블라인드 6회차(7/26·5/20·5/27·6/01·6/04·5/11)의 스캔 캐시를 패널에서 생성해 코퍼스에 합류시켰다. **84 → 90회차**(라벨 파일·캐시 모두). 여섯 회차 전부 `DEFAULT_HOLDOUT`에 넣어 학습이 삼키지 못하게 고정했다(홀드아웃 7→13회차).

- **오프라인 전량**: 90회차 TP 997 · FP 38 · FN 34 → **F1 96.5**(84회차 96.8에서 −0.3, 새 회차의 오프라인 결손이 반영된 값 — 실기에서는 이 여섯 회차 전부 F1 100이다).
- **홀드아웃 재측정**: 커밋 모델이 13회차 홀드아웃에서 **P 92 R 86 F1 88.7**(7회차 홀드아웃 시절 85.5보다 높다 — 새로 들어온 6회차를 잘 맞히기 때문).
- **재학습 결과**: 스윕 6종 최고가 **84.7**로 커밋 모델 88.7을 못 이겨 **모델 파일 미변경**. 즉 **데이터를 6회차 늘려도 무료 경로 모델은 개선되지 않는다**.

§116(특징 추가 +0.7·자기 일관성 기각)과 합치면, 오늘 시도한 학습 레버는 **하이퍼파라미터·특징·데이터량 세 갈래 모두 소진**이다. 무료 경로 모델은 현 구조(147차원 로지스틱)에서 천장에 닿았고, 실기 F1 100은 모델이 아니라 **비전 검증·회수·프롬프트**가 만들고 있다. 다음 모델 개선을 시도한다면 표현 자체를 바꿔야 한다(§109에서 보류한 TransNetV2류 또는 진짜 띠 벡터를 담는 캐시 재생성).

## §119 블라인드 7회차 전원 F1 100 — out-of-sample 2연속 (2026-07-28)

| # | 회차 | 성격 | 정답 | TP/FP/FN | **F1** |
|---|---|---|---|---|---|
| 1 | 7/26(일) | 진짜 블라인드 | 11 | 11/0/0 | 100 |
| 2 | 5/20(수) | 진짜 블라인드 | 11 | 11/0/0 | 100 |
| 3 | 5/27(수) | 진짜 블라인드 | 14 | 14/0/0 | 100 |
| 4 | 6/01(월) | in-sample(절 설계에 사용) | 10 | 10/0/0 | 100 |
| 5 | 6/04(목) | in-sample(절 설계에 사용) | 8 | 8/0/0 | 100 |
| 6 | 5/11(월) | **out-of-sample** | 14 | 14/0/0 | 100 |
| 7 | **5/07(목)** | **out-of-sample** | 12 | 12/0/0 | **100** |

- 누적 **80아이템 · FP 0 · FN 0**. 현재 빌드에서 실기 비전 ON 블라인드 7회차가 모두 만점이다.
- 5/07은 서브에이전트가 **0.25초 브래킷으로 12경계 전부를 프레임 확정**했고(직전 컷 마지막 프레임 ↔ 앵커 첫 프레임 대조), 최장 4구간을 2초 간격으로 재스윕해 짧은 앵커샷 누락이 없음까지 확인했다. 관측된 앵커 블록 최단 길이 약 8초라 4초 시트 판독이 놓칠 위험은 없다.
- 파이프라인 경계는 정답 대비 **최대 2.0초 차**(190 vs 188 등)로 전부 ±8초 안. 후보 12개가 곧 정답 12개였고 회수·배제 개입이 없었다.
- 판정 정직성: 4·5번(6/01·6/04)은 단일 진행자 절을 그 회차 FP로 설계했으므로 in-sample이다. **일반화 근거는 6·7번(5/11·5/07)** 두 회차다.

## §120 8회차에서 첫 붕괴 — 오염된 표 제거로 회복 (2026-07-28)

블라인드를 계속 늘리던 중 **8회차(5/08 금)에서 처음으로 F1 100이 깨졌다**. 그리고 그 원인이 오늘 만난 다른 FP들과 성격이 달랐다.

- **증상**: 5/08 정답 13아이템을 전부 잡았으나(재현율 100) 382초에 경계가 하나 더 생겨 **F1 96.3**. 프레임 확인 결과 380초에 앵커 블록이 끝나고 유조선 항공샷이 시작되는 지점 — 명백한 FP.
- **기전**: 이 회차의 진짜 앵커 블록이 **360~380초로 20초**나 길었다. 382 후보의 **−3초 프로브가 379초** = 직전 앵커 블록 한가운데에 떨어져 "앵커" 표를 찍었고, §92의 4표 전원 합의가 깨져 배제가 무산됐다. 로그도 "전 후보 앵커 확인(제외 0)"으로 일치한다.
- **다른 FP와의 차이**: 대담 게스트·협약식·개표방송은 **비전이 헷갈린** 경우라 프롬프트로 풀렸지만, 이번은 **비전은 옳게 봤는데 투표 규칙이 오염된 표를 셌다**. 프롬프트로는 영원히 안 잡히는 유형이다.
- **수정**: 후보의 −3초 지점이 앞선 후보의 앵커 구간 안이면 그 후보만 **−3초를 빼고 남은 3표 전원 합의**로 배제한다. 표를 덜 요구하는 게 아니라 오염된 표를 빼는 것이라 오배제 위험은 오히려 낮다(진짜 앵커는 나머지 3표도 앵커).
- **1차 수정이 무효였던 기록**: 누수 창을 `직전 후보 +8초`로 잡았더니 379초(직전 후보 362로부터 17초)를 못 덮어 재실행에서 FP가 그대로였다. **창을 25초로 넓혀야** 발동했다 — 앵커 블록 길이(10~30초, §112 시맨틱)를 창이 덮어야 한다는 뜻.

| 회차 | 수정 전 | **수정 후** |
|---|---|---|
| 5/08(금) | 96.3 (FP 382) | **100** (13/13) |
| 5/07 회귀 | 100 | **100** 유지 |
| 5/11 회귀 | 100 | **100** 유지 |

**블라인드 누적 8회차 · 93아이템 · 현재 빌드에서 전원 F1 100.**


## §121 F1 100은 10회차까지 — 11회차에서 짧은 앵커 블록에 무너졌다 (2026-07-28)

블라인드를 계속 늘려 한계를 실측했다. **F1 100은 10회차까지 버텼고 11회차(3/24 화)에서 깨졌다.**

| # | 회차 | F1 | 아이템 | 비고 |
|---|---|---|---|---|
| 1~8 | 7/26·5/20·5/27·6/01·6/04·5/11·5/07·5/08 | 100 | 93 | 5/08은 §120 수정 후 |
| 9 | 4/20(월) | 100 | 14/14 | 비전 오배제 1건을 회수가 자가치유(아래) |
| 10 | 4/14(화) | 100 | 14/14 | 배제 1(234, 정당)·회수 1(678) |
| 11 | **3/24(화)** | **96.3** | 13/14 | **FN 920.4** |
| 12 | 6/26(금) | 100 | 14/14 | 배제 0·회수 0 |

### 붕괴 기전 — §110 전제가 깨지는 조합

3/24 마지막 아이템은 **앵커 블록이 920~927초로 8초뿐**이고, 하단 배너가 그 안에서 **923초에야** 떴다. §110의 `detectBandEvents`는 "띠가 바뀌고 이후 4표본이 안정된 지점"을 이벤트로 삼는데, 배너 등장(924 표본) 직후 928에서 스튜디오→도로 b-roll 컷이 나 안정 조건이 깨졌고, 이벤트는 컷 **이후** 930초에 잡혔다. 930은 이미 b-roll이라 비전이 정확히 "비앵커 0.98"로 답했고 회수가 실패했다.

즉 §110 주석의 전제 — "띠 이벤트 시각은 띠가 바뀐 직후 표본이라 이미 새 아이템 리드 안" — 은 **짧은 앵커 블록 + 늦은 배너**에서 성립하지 않는다. 이벤트는 리드인의 시작이 아니라 **끝 이후**를 가리킬 수 있다.

이 회차가 특히 취약했던 배경도 남긴다. **학습 모델이 이 회차에서 시작을 하나도 내지 못했다**(후보 7개 전부 참조 뱅크 매칭, 모델 0). 최종 13개 중 6개를 띠 회수가 만들었으므로 회차 전체가 회수 경로 하나에 걸려 있었다.

### 수정 — 되짚기 2차 라운드

1차에서 앵커를 못 찾은 **띠 이벤트 지점만** 골라 6초 앞을 한 번 더 본다. 되짚기 거리 6초는 실측에서 나왔다 — 930−6=924는 앵커 블록(920.4~927) 안이고, 재스냅이 뒤로 훑어 920.4로 정확히 당긴다. 10초로 잡으면 920이라 컷 직전으로 새어 오히려 위험하다.

§107(무차별 격자 훑기 → 비경고 회차 FP 3건으로 원복)과 다른 점은 **사정권**이다. 훑는 곳이 "하단 띠 전환이 실제로 있었고 1차가 빗나간 지점"뿐이라 임의 b-roll 노출이 늘지 않는다. 비용은 실패한 띠 프로브 1개당 프레임 1장(회차당 10~13장).

### 곁가지 — 4/20 846 비전 오배제(F1 영향 없음, 미규명)

4/20에서 비전이 846초(진짜 앵커, 라벨 844.9)를 4표 전원 합의로 배제했고 회수가 848로 되살려 F1은 100이 됐다. §92 "오배제 0" 조건에 걸리는 사건이라 4단으로 규명했다.

- 모델 단독 4프레임 재질의: **3/3 정확**(843 비앵커 0.99 · 847.2 앵커 0.96~0.98 · 850 앵커 0.95~0.98 · 853 비앵커 0.98)
- 참조 주입 경로: 이 프로젝트 localStorage의 앵커 예시가 **0개**라 운영도 참조 없이 호출했다 — 원인 아님, 3/3 정확
- **운영과 동일한 12장 배치 문맥**(806 잔여 3 + 846의 4 + 882의 4 + 918의 1): **3/3 전원 12/12 정확**, 846 배제표 2/4
- 투표 코드·어댑터 인덱스 처리 정독: 정상(어댑터가 모델 인덱스를 보존하고 범위검사, 유실은 표 부족으로 FP 생존 쪽으로만 작용)

남는 결론은 **운영이 그 후보로 보낸 프레임이 그 시각의 프레임이 아니었다**는 것이다. 확인 프로브에서 `ppro.Exporter.exportSequenceFrame`이 **true를 반환하고도 파일을 남기지 않는** 상태를 재현했다 — 내보내기 충실도가 다음 조사 대상이다. F1에는 영향이 없어 §121 검증 뒤로 미뤘다.

### 측정 위생

- 블라인드 판독은 전 회차 서브에이전트 1차 + 본인 재검증으로 했고, **시트 타임스탬프가 회차마다 ±1~3초 어긋나는 방향이 제각각**임을 다시 확인했다(4/20 일관되게 −1.5s, 6/26 일관되게 +1.5~2s). 시트 값을 그대로 라벨로 쓰면 전 경계가 한 방향으로 밀린다.
- 6/26의 1011.8 → 1007.5는 Δ4.3초로 **허용치 ±8에 가깝다**. 재스냅이 전 컷으로 지나치게 당긴 사례이므로 관측 대상으로 남긴다.
- 5/07(12아이템)·5/08(13아이템)을 코퍼스에 등록해 홀드아웃이 **15회차**가 됐다. 오프라인 지표 ②는 P92 R88 **F1 90.2**(등록 전 13회차 88.7). 5/07을 라벨만 넣고 `DEFAULT_HOLDOUT`을 빠뜨렸다가 커밋 직전에 잡았다 — §100의 함정은 지금도 살아 있으니 등록과 홀드아웃 갱신은 반드시 한 커밋에서 할 것.

### §121-b 재실행이 드러낸 것 — 결함 2개와 실행 간 변동

되짚기를 넣고 3/24를 재실행하자 F1은 여전히 96.3이었는데, **놓친 아이템이 920에서 248로 바뀌었다.** 원인을 셋으로 갈랐다.

1. **되짚기 배치가 바이트 상한을 안 지켜 전량 실패했다.** 로그에 "회수 되짚기 배치 실패 — 앵커 샷 분류 프레임 합계가 너무 큽니다"가 2회. 장수(rescuePerBatch)로만 나누고 어댑터 1.2MB 캡을 무시한 탓이다. §101-b에서 같은 실수로 회수가 0건이 된 적이 있는데 새 경로에서 되풀이했다 — 기존 두 경로와 동일한 바이트 기준 분할을 넣었다.
2. **248은 탐지 실패가 아니라 내보내기 유실이었다.** "회수 훑기 · 프레임 내보내기 유실 1장: 252" — 252 프로브가 빈손으로 끝나 그 지점 회수 기회가 사라졌다. 검증 경로(§92)에는 내보내기 2회 재시도가 있는데 **회수 경로에는 없었다.** 회수·되짚기 양쪽에 같은 재시도를 넣었다.
3. **같은 빌드·같은 회차인데 후보가 달라진다.** 1차 실행 "화면 매칭 후보 20 → 확정 7", 2차 "22 → 9". 띠 이벤트도 19곳 vs 15곳이고, 문제의 이벤트가 1차엔 930(컷 이후), 2차엔 924(앵커 블록 안)로 잡혔다 — 2차는 그래서 920을 회수했다. §59의 "후보 생성 비결정성"이 이 정밀도에서는 **회차 판정을 좌우하는 크기**라는 뜻이다.

3번은 목표 자체에 영향을 준다. "미학습 신규 회차 3연속 F1 100"을 실행 1회로 판정하면 안 된다 — 같은 회차가 실행마다 다른 아이템을 놓칠 수 있다. 판정은 **회차당 복수 실행**으로 해야 한다.

### 다음 후보 (아직 미적용)

- **모델 기여 0인 회차를 경고 회차로 승격.** 3/24는 `bankFit`이 경고 문턱 아래라 격자 회수가 꺼져 있었는데(격자 0), 정작 **학습 모델이 시작을 하나도 못 냈다**. "학습 범위 밖"의 가장 직접적인 신호를 트리거로 안 쓰고 있는 셈이다. §107-b(무차별 격자 → FP 3건) 때문에 조건을 좁게 잡아야 하며, 모델 0은 충분히 좁다. §121 되짚기 검증이 끝난 뒤 단독으로 실측할 것.
- **Premiere 내보내기 충실도.** 확인 프로브에서 `exportSequenceFrame`이 true를 반환하고도 파일을 안 남기는 상태를 재현했다. 재시도로 증상은 덮었지만 원인은 미규명이다.

### §121-c 되짚기 원복 — 거리로는 "긴 앵커 블록"과 "새 단신"을 가를 수 없다

되짚기 2차 라운드를 실기 2회 시험하고 **원복했다**(내보내기 재시도만 남긴다).

| 실행 | 3/24 결과 | 되짚기가 만든 것 |
|---|---|---|
| 되짚기 1차(창 20s) | 15아이템 · **FP 78** | 78→앵커(0.98) 채택, 308·466은 20초 창에 걸려 기각 |
| 되짚기 2차(창 30s) | 15아이템 · **FP 90** | 78은 창에 걸렸으나 90→앵커(0.99)가 통과 |
| 6/26 회귀 | 14아이템 · FP 0 | 9곳 전부 비앵커 — 무해 |

78도 90도 **비전이 옳았다**. 둘 다 진짜 앵커샷이고, 아이템 1(시작 52) 의 앵커 블록이 90초 넘게 이어지는 것이었다(대담 예고 리드인). 즉 되짚기가 찾은 것은 새 경계가 아니라 **같은 앵커 블록의 뒷부분**이다. 중복제거 창을 20→30초로 넓혀도 블록이 38초를 넘으면 또 샌다 — **거리로는 원리적으로 못 가른다.** 창을 더 넓히면 이번엔 진짜 단신(관측 최단 간격 31초, 6/26)을 죽인다.

한편 되짚기가 없어도 3/24는 14/14가 나온다. 4회 실행 중 띠 이벤트가 컷 이후(930)에 잡힌 것은 1회뿐이고, 나머지 3회는 924에 잡혀 1차 회수가 그대로 성공했다. 즉 되짚기의 이득은 "4회 중 1회의 FN 방지"인데 대가는 "3/24에서 2회 중 2회 FP"였다. §107-b·§111-a와 같은 판정 — **무승부 이하는 원복한다.**

**재도전 조건**: 되짚기 발견이 직전 시작과 같은 앵커 블록인지를 거리가 아니라 **연속성으로** 판정해야 한다. 후보는 중간점 1장 추가 판정이다 — 직전 시작과 되짚기 발견의 중간이 앵커면 같은 블록(3/24: (52+78)/2=65 → 앵커 → 기각), 비앵커면 새 아이템(3/24: (866+924)/2=895 → b-roll → 채택). 발견 1건당 프레임 1장이라 비용도 작다.

**남긴 것**: 회수·되짚기 경로의 내보내기 2회 재시도(§121-b ②)는 유지한다. 이건 무조건 이득이다 — 252 프로브 유실 1장이 248 FN으로 직결된 실측이 있고, 되돌릴 이유가 없다.

### 사고 기록 — Python 텍스트 쓰기가 index.ts 전체를 CRLF로 바꿈

원복 스크립트를 Python `io.open(p, "w")`로 썼더니 Windows 기본 개행 변환으로 **파일 전체가 LF→CRLF**가 됐고, `tests/ui-contract.test.ts`의 다중행 정규식(`/
\}

function guarded/u`)이 깨져 테스트 1건이 실패했다(1842→1841 pass, fail 1). 타입체크·린트는 통과해서 원인이 안 보였다. 전역 CLAUDE.md가 경고한 그 함정이다 — **다중행 치환은 바이너리 모드로 읽고 쓰거나 Edit 도구를 쓸 것.** 복구는 `
`→`
` 일괄 치환.

### §121-d 최종 판정 — 내보내기 재시도만으로 3/24가 복구됐다

원복 빌드(되짚기 제거 + 회수 경로 내보내기 재시도)로 3/24를 **2회 연속 실행**했다.

| 실행 | 후보 | 배제 | 회수 | 결과 |
|---|---|---|---|---|
| 1차 | 10 (모델 0) | 2 (524·708 — 둘 다 정당) | 196·252·404·696·800·924 | 14/14 **F1 100** |
| 2차 | 9 (모델 0) | 1 (524) | 196·252·404·696·800·924 | 14/14 **F1 100** |

두 실행 모두 후보 수가 다른데도(10 vs 9) 최종 14아이템이 같았다 — 회수 경로가 후보 변동을 흡수했다. **3/24는 §121-d에서 해결된 것으로 본다.**

블라인드 최종 성적(12회차 135아이템): 3/24를 포함해 **전 회차 F1 100**. 단, 이것은 회차별 1~4회 실행의 종합이며 §121-b의 실행 간 변동을 감안하면 "달성"이 아니라 "현재 관측치"다.
