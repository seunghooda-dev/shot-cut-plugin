...
- **영역 제한 매처 예비 결과(재학습 포함, 홀드아웃 5회차)**: 열6 행0~8이 **7/23을 예측 5개→12개(정답과 동수), F1 47.1→75.0**으로 회복시켰고 홀드아웃 전체도 75.9→78.3. 다만 7/11은 93.3→82.4, 7/12는 90.0→88.9로 **손실도 있다**. 또 7/23 회복은 후보 생성이 한 것이고 모델 시작은 1개뿐이라 **모델은 여전히 뱅크에 얹혀 있다**. 하이퍼파라미터 한 세트·홀드아웃 5회차 결과이므로 채택 전 전량 회귀와 스윕이 필요하다(진행 중).
