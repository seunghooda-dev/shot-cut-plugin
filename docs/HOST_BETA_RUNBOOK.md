...
- **처방 방향(수정됨)**: ②"모델에서 뱅크 거리 특징을 떼면 된다"는 **단순 제거로는 불가능하다** — 위 실측대로 제거 즉시 모델이 무차별 검출로 무너진다. 남는 선택지는 ①단기 — 7/23류 앵커 프레임을 참조 뱅크에 추가(즉효, 다만 새 회차마다 반복되는 대증요법이고 배포판 사용자는 뱅크를 갱신할 수단이 없다) ②'구조 — 뱅크 거리 **없이** 처음부터 재학습해 luma 가중치가 실제 판별력을 갖도록 만들고, 홀드아웃으로 가능 여부 자체를 검증(가능하다는 보장이 없으므로 실험이 선행돼야 한다) ③운영 — 뱅크 최소거리가 임계(예: 0.10)를 넘으면 "이 회차는 학습 범위 밖일 수 있다"고 경고하고 사용자가 수동 보정할 여지를 준다. 어느 쪽이든 **홀드아웃을 날짜 무작위가 아니라 "코퍼스에 없는 최신 회차"로 잡아야** 이 실패가 게이트에 걸린다.
