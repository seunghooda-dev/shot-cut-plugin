...
| 지표 | 뜻 | 2026-08-03 실측 |
|---|---|---|
| ① 코퍼스 전량(115회차) 오프라인 | 무료 경로, 학습셋 포함이라 낙관적 | **92.8** (§177 4회차 편입 후 무하락) |
| ② **홀드아웃**(28회차) | 무료 경로 일반화 | P 90 R 84 **F1 86.8** (§177로 24→28회차 확장 — 종전 85.5와 분모 상이) |
| ③ **실기 비전 ON 블라인드** | **이것이 목표 대상** | **45회차 중 43회차 100**(§190-c에서 7/23 완치·7/09 편입) · 미달 2: 4/07 96.3(§125 4형, §188-b 종결) · 7/09 96.0(대담 코너 — 신규 구조 유형, §190-c). 현행 빌드 재확인은 §190-c 기준 18회차 — 아래 주의 참조 |

**①이 96.5 → 92.7 → 92.8로 움직인 것은 회귀가 아니다.** 어려운 회차·신규 회차를 코퍼스에
편입해 분모가 계속 바뀌었다(90 → 111 → 115회차). **①이 낮은 회차가 실기에서 나쁜 것도
아니다** — 2/10은 ①이 10.5인데 실기 100이고, 3/03·3/12도 ① 16.7에 실기 100이다.
...
③은 2026-07-31에 41회차로 확장 실측했고(§156~§159), 8/1·8/2 편입(§175·§175-b)으로
**43회차 중 42회차 100 · FP 0**이 됐다 — 7/29 데스크 칼럼은 §170에서 닫혔다(82.8 →
**5실행 전부 100**). 남은 미달은 **4/07 96.3(§125 4형) 하나**이고 실행 간 동일값이라
요동이 아니라 구조 상수다. **진짜 미학습 신규 회차** 7/27·7/28·7/29·7/30·7/31·8/1·8/2
7회차는 전부 복수 실행 100이다. 과거 요동 이력 회차(2/10 76.9↔100 · 1/06 FP 250 ·
2/19 FN)는 2실행씩 전부 100으로 **요동이 소멸했다**(§156).

**"42회차 100"은 여러 빌드에 걸친 누적치다.** 현행 빌드(§176-c 원복 후 = §170-d와 동등
로직)로 실제 재확인한 것은 13회차다 — 7/27·7/28·7/29·7/30·7/31(신규) · 1/06·2/10·2/19·
3/10·5/08·1/20·3/24(과거 FP·요동 이력) · 4/07(4형 감시); 이 중 4/07·1/06·7/27은 2026-08-03
체인65에서도 재확인했다(96.3·100·100). 8/1·8/2는 §168 계열 빌드의 3실행 100이다. 나머지는
이전 빌드의 결과이고, 사용자 지시("전량 재검증 금지, 표적+대조 소수로 판정")에 따라
재검증하지 않는다. **회귀가 의심되면 전량이 아니라 과거 FP 이력 회차부터 본다** — 회수
경로의 위험은 깨끗한 회차에서 드러나지 않는다.
...
| 유형 | 대표 | 상태 |
|---|---|---|
| §125 4형 (앵커 VO + b-roll 직행) | 4/07 220.8 | **수단 소진 최종 확정(§188-b, 열째 경로까지 기각).** 여덟 경로(§171)에 이어 아홉째(§176 비전 주제 판정)는 실기 기각·원복, 열째(§188 로컬 5중 게이트)는 **사전 실측 통과 후 신호원 검토에서 기각** — 표적/대조를 가른 공백 게이트의 물리적 실체가 "직전 b-roll에 우연히 간판이 있었는지"(OCR 크롭에 걸린 배경 텍스트)였고, 1080p 육안으로 74 직전과 224 직전은 동일한 띠 부재 상태라 어떤 강건 신호원으로도 구별 불가. 밴드 luma·에지 대체도 79%가 상한. 4/07 96.3은 구조 상수다. 다시 파려면 §188-b부터 읽을 것 |
...
- `npm run check:mw` — 모닝와이드 오프라인 회귀 게이트(학습 13 + 홀드아웃 6 = 19회차). check:news와 달리 **제품 모듈을 그대로 불러 쓴다**(`.test-build` 컴파일 산출물이라 뱅크 라우팅·P5·합집합이 제품과 같은 코드). 모닝와이드 로직·뱅크·모델·문턱 수정 후 반드시 실행하고, **스냅샷은 같은 커밋에서 다시 잰다.** 스캔 캐시(`training-data/morning-wide/scan-cache/`, gitignore)가 없으면 건너뛴다.
...
확인하지 않는다. **적용 범위는 8뉴스와 모닝와이드 둘 다이다(2026-08-04 사용자 지시) —
모닝와이드도 목표는 F1 100과 정밀도 상승이고, 24시간 365일 끊이지 않고 진행한다.
단 두 프로그램의 지표·코퍼스·홀드아웃은 절대 혼용하지 않는다(morning-wide-split.plan.md §4).**
...
1. **멈추지 않는다 — 보고보다 착수가 먼저다.**
   **순서를 반드시 지킨다: ①다음 작업을 먼저 건다 → ②그다음에 보고 문장을 쓴다.**
   ...
   - **작업을 완료하면 반드시 다음 작업을 먼저 시작한 뒤에 보고를 쓴다**(2026-08-04 사용자
     지시로 재명문화 — "작업 완료 후엔 다음 작업 시작하고 나서 보고").
2. **정기 점검 스케줄을 건다 — 1시간마다**(2026-08-02 사용자 지시로 2시간 → 1시간 격상,
   24시간 365일 체제. 2026-08-04 사용자 지시로 "작업 제대로 하고 있는지"의 알림·보고를
   매시 점검에 포함 — 세션이 재시작되면 CronCreate 1시간 점검부터 다시 건다).
   ①아무것도 실행 중이 아니면 즉시 다음 작업 기동(점검의 제1목적)
   ②직전 완료분과 수치 ③로테이션·디스크·8월분 업로드 검문. 보고 후 대기하지 말고 이어서
   진행한다. 공백 1시간 초과 금지.
   **[2026-08-08 사용자 지시로 스케줄러 격상]** — 매시 점검은
   [docs/OPERATIONS_SCHEDULER.md](docs/OPERATIONS_SCHEDULER.md)의 우선순위 큐(P0~P4)를
   평가해 기동한다. 병목(사용자 대기·크레딧·신규 없음·열화)이 생기면 그 조건이 필요 없는
   작업으로 즉시 전환 — "변화 없음" 보고만 남기는 유휴 사이클 금지, 백로그 3개 이상 상시 유지.
3. **5시간마다 새 Premiere 프로젝트로 전환하고 시퀀스를 정리한다.** 과부하 예방이 목적이다.
   **8뉴스·모닝와이드 공용 규칙이다**(2026-08-04 사용자 지시 — 모닝와이드 실기도 같은
   로테이션·아이템 시퀀스 정리 대상). 전환은 **회차 경계에서만**
   한다(실행 중간 전환은 §40-d 단일 세션 원칙을 깨고 그 회차를 무효로 만든다).
   도구는 scratchpad `run-rotating.sh`(자동) · `cdt-rotate-project.mjs`(수동) ·
   `cdt-cleanup-seqs.mjs`(시퀀스 정리).
4. **승인이 필요한 지점에서는 권장안으로 진행한다.** 묻고 기다리지 않는다. 다만 되돌릴 수 없는
   작업(파일 영구 삭제, 원격 푸시 외의 외부 발신)은 예외이며, 판단 근거를 보고에 남긴다.
5. **서브에이전트는 항상 최대로 고용한다**(2026-08-03 지시 → 2026-08-04 재강조 →
   **2026-08-09 사용자 지시로 최종 확정: "서브에이전트는 항상 최대로 고용한다"**).
   순차 처리가 기본값이 아니다 — 매 사이클 "지금 병렬로 돌릴 수 있는 조사·감사·판독이
   무엇인가"를 먼저 묻고 여러 명을 동시에 띄운다. 단 **판정·확정은 본인 몫**이고(라벨 확정은
   본인 재검증 필수), 실기(단일 CDP 세션)와 빌드는 위임하지 않는다. 상세 규약은
   [docs/OPERATIONS_SCHEDULER.md](docs/OPERATIONS_SCHEDULER.md) 원칙 4항.
   프레임 판독·대량 탐색·
   감사·보고서 작성처럼 실기(단일 CDP 세션)와 자원이 겹치지 않는 일은 가능한 한 병렬로
   에이전트에 맡긴다. 종류 제한 없다 — Explore·general-purpose·Plan 등 그 일에 맞는 것을
   쓴다. 필요하면 **외부 자료·GitHub 레퍼런스 참고도 허용**된다(웹 검색·문서
...
7-a. **4시간마다 작업 계획을 세운다**(2026-08-03 /goal). 매 4시간 사이클마다 ①직전 4시간
   ...
   먼저 실행한다**(1항과 동일 원칙 — 계획·보고는 실행 중인 작업 위에서 쓴다).
6-a. **방송분(blind 신규 업로드) 확인은 밤 12시(자정) 1회만 한다**(2026-08-04 사용자 지시).
   매시 점검에서는 blind 폴더를 확인하지 않고 다른 작업 계획을 우선한다. 자정 확인에서
   신규가 있으면 §151 오염 검사 후 3실행·검문·채점으로 전환한다.
   **[2026-08-04 저녁 지시로 당분간 보류]** — "8뉴스는 당분간 그대로 두고 모닝와이드
   테스트만 진행"에 따라 **8뉴스 신규 회차 확인·실기 3실행을 멈춘다.** 자정 확인도
   재개 지시 전까지 하지 않는다.

6-b. **당분간 실기 테스트 대상은 모닝와이드뿐이다**(2026-08-04 저녁 사용자 지시).
   - 8뉴스는 **동결** — 신규 실기 측정·블라인드 확장·프롬프트 튜닝을 하지 않는다.
   - **단, 회귀 방어는 계속한다.** 공용 코드(`news-visual-cut.ts`·`openai-text.ts`·
     `index.ts` 분할 경로)를 건드리면 `npm run check:news`를 반드시 통과시킨다 —
     "그대로 둔다"는 새 작업을 멈춘다는 뜻이지 8뉴스가 깨져도 좋다는 뜻이 아니다.
   - 8뉴스 자산(45회차 명단·비전 벤치마크 477장)은 회귀 판정용으로 유지한다.
7. **개선 작업은 끝이 없다 — "할 일 없음"은 존재하지 않는다**(2026-08-02 사용자 지시:
   "이건 끝이 없는 작업이야, 개선 작업을 24시간 항상 한다"). 명시된 작업 큐가 비면 대기
   모드로 전환하지 말고 다음 발굴 루프를 돈다. ①안정화 감사(코드·문서·운영 신호)
   ②테스트 공백 탐색 ③실측 로그의 미해결 요동 추적 ④문서-제품 정합 재검 ⑤디스크·캐시
   위생. 발굴 → 우선순위 → 착수를 반복하며, 각 사이클의 판단 근거를 기록한다.
